...
const XhsFreeRoamApp: React.FC = () => {
    ...
    const [pickerGroupId, setPickerGroupId] = useState<string>(GROUP_FILTER_ALL); // 角色下拉選擇器的分組篩選（須在組件頂層，勿移入 renderCharPicker）
    ...
    const handleStart = async () => {
        ...
        if (!mcpEnabled || !mcpUrl) {
            addToast('請先在設置中配置小紅書 MCP Server', 'error');
            return;
        }
        if (!apiConfig.baseUrl) {
            addToast('請先在設置中配置 API', 'error');
            return;
        }
        ...
        const callbacks: FreeRoamCallbacks = {
            ...
            onComplete: (session) => {
                setStatus(`活動結束: ${session.summary || '完成'}`);
                setIsRunning(false);
                loadActivities();
                addToast(`${char.name}的自由活動結束了`, 'success');
            },
            onError: (err) => {
                setStatus(`出錯: ${err}`);
                setIsRunning(false);
                addToast(`自由活動出錯: ${err}`, 'error');
            },
        };
    ...
    const handleClearHistory = () => {
        ...
        setConfirmDialog({
            isOpen: true,
            title: '清除活動記錄',
            message: `確定清除${char.name}的所有小紅書活動記錄嗎？`,
            variant: 'danger',
            onConfirm: async () => {
                await DB.clearXhsActivities(char.id);
                setActivities([]);
                setConfirmDialog(null);
                addToast('記錄已清除', 'success');
            }
        });
    };
    ...
    const handleLoadClaimCandidates = async () => {
        ...
        if (!mcpEnabled || !mcpUrl || !userId) {
            addToast('請先在設置中連接小紅書並獲取登錄用戶 ID', 'error');
            return;
        }
        ...
        try {
            ...
            if (!result.success) throw new Error(result.error || '真實帳號主頁讀取失敗');
            ...
            addToast(error?.message || '讀取真實帳號帖子失敗', 'error');
        } finally {
    ...
    const handleClaimPost = async (note: ReturnType<typeof normalizeNote>) => {
        ...
        addToast(`已歸入${char.name}的主頁`, 'success');
    };
            ...
            <div className="fixed inset-0 z-50 bg-black/30" onClick={() => setShowCharPicker(false)}>
                <div className="absolute top-14 left-4 right-4 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden animate-fade-in" onClick={e => e.stopPropagation()}>
                    <div className="p-3 border-b border-slate-50">
                        <p className="text-xs font-bold text-slate-400">選擇角色</p>
                        {/* 分組篩選（沒建分組時不渲染），白底下拉走淺色 */}
        ...
        return (
            <div className="h-full flex flex-col bg-gradient-to-b from-rose-50 to-white">
                <div className="flex items-center px-4 py-3 border-b border-slate-100" style={{ paddingTop: 'max(0.75rem, var(--safe-top))' }}>
                    ...
                    <h1 className="text-base font-bold text-slate-800 ml-1">自由活動</h1>
                </div>
                <div className="flex-1 flex items-center justify-center">
                    <p className="text-sm text-slate-400">還沒有角色，請先創建角色</p>
                </div>
            </div>
        );
    }
            ...
            >
                <div className="w-full max-w-lg bg-white rounded-t-3xl p-5 space-y-3 max-h-[75vh] overflow-y-auto animate-slide-up" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="bg-violet-50 rounded-2xl p-3">
                        <p className="text-[10px] font-bold text-violet-400 mb-1">內心想法</p>
                    ...
                    {a.content.title && (
                        <div className="bg-slate-50 rounded-2xl p-3">
                            <p className="text-[10px] font-bold text-slate-400 mb-1">標題</p>
                    ...
                    {a.content.keyword && (
                        <div className="bg-blue-50 rounded-2xl p-3">
                            <p className="text-[10px] font-bold text-blue-400 mb-1">搜索關鍵詞</p>
                    ...
                    {a.content.notesViewed && a.content.notesViewed.length > 0 && (
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-bold text-slate-400">瀏覽過的帖子</p>
                    ...
                    {a.content.savedTopics && a.content.savedTopics.length > 0 && (
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-bold text-amber-500">保存的話題</p>
                    ...
                    {a.content.commentText && (
                        <div className="bg-green-50 rounded-2xl p-3">
                            <p className="text-[10px] font-bold text-green-500 mb-1">評論內容</p>
                            <p className="text-xs text-green-700">{a.content.commentText}</p>
                            {a.content.commentTarget && (
                                <p className="text-[10px] text-green-500 mt-1">對「{a.content.commentTarget.title}」的評論</p>
                            )}
                        </div>
                    )}
                    ...
                    <button
                        onClick={() => {
                            setConfirmDialog({
                                isOpen: true,
                                title: '刪除此條記錄',
                                message: `確定刪除這條${ACTION_LABELS[a.actionType] || '活動'}記錄嗎？`,
                                variant: 'danger',
                                onConfirm: async () => {
                ...
                <div className="w-full max-w-lg bg-white rounded-t-3xl px-5 pt-5 pb-6 max-h-[78vh] overflow-y-auto animate-slide-up" onClick={e => e.stopPropagation()}>
                    <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                            <p className="text-[10px] font-bold tracking-[0.16em] text-rose-400 uppercase">{char?.name}的筆記</p>
                            <h2 className="text-lg font-bold text-slate-900 mt-1 leading-snug">{post.title || '無標題'}</h2>
                        </div>
                        <button onClick={() => setShowOwnedPost(null)} className="text-xs text-slate-400 px-2 py-1 active:text-slate-700">關閉</button>
                    </div>

                    <p className="text-sm text-slate-600 leading-7 whitespace-pre-wrap mt-5">{post.body || '（沒有正文）'}</p>
                    ...
                    <div className="grid grid-cols-3 gap-4 border-y border-slate-100 py-4 mt-6 text-center">
                        <div><p className="text-base font-bold text-slate-800">{post.likes || 0}</p><p className="text-[10px] text-slate-400">贊</p></div>
                        <div><p className="text-base font-bold text-slate-800">{post.collects || 0}</p><p className="text-[10px] text-slate-400">收藏</p></div>
                        <div><p className="text-base font-bold text-slate-800">{post.commentCount || 0}</p><p className="text-[10px] text-slate-400">評論</p></div>
                    </div>

                    <div className="mt-4 space-y-1 text-[10px] text-slate-400">
                        <p>發佈於 {new Date(post.publishedAt).toLocaleString()}</p>
            ...
            >
                <div className="w-full max-w-lg bg-white rounded-t-3xl max-h-[82vh] flex flex-col animate-slide-up" onClick={e => e.stopPropagation()}>
                    <div className="px-5 pt-5 pb-4 border-b border-slate-100">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                <h2 className="text-base font-bold text-slate-900">認領舊帖子</h2>
                                <p className="text-[11px] text-slate-500 mt-1">從共享真實帳號中，選擇由{char?.name || '該角色'}發佈的帖子。</p>
                            </div>
                            <button onClick={() => setShowClaimPicker(false)} className="text-xs text-slate-400 px-2 py-1 active:text-slate-700">關閉</button>
                        </div>
                    </div>
                    ...
                    <div className="overflow-y-auto min-h-0">
                        {claimCandidates.length === 0 ? (
                            <div className="px-6 py-14 text-center">
                                <p className="text-sm font-medium text-slate-500">沒有可認領的帖子</p>
                                <p className="text-[11px] text-slate-400 mt-1">帳號帖子可能已經歸屬其他角色，或主頁暫時沒有筆記。</p>
                            </div>
                        ) : claimCandidates.map(note => (
                            <div key={note.noteId} className="px-5 py-4 border-b border-slate-100 flex items-start gap-4">
                                <div className="min-w-0 flex-1">
                                    <p className="text-sm font-bold text-slate-800 line-clamp-2">{note.title || '無標題'}</p>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0">
            ...
            {liveActivities.map((a, i) => (
                <div key={a.id || i} className="bg-white rounded-2xl border border-slate-100 p-3 space-y-1.5 animate-fade-in">
                    <div className="flex items-center justify-between">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${RESULT_COLORS[a.result]}`}>{a.result === 'success' ? '完成' : a.result === 'failed' ? '失敗' : '跳過'}</span>
                    </div>
            ...
            {isRunning && liveActivities.length === 0 && !thinking && (
                <div className="flex flex-col items-center justify-center py-12 opacity-50">
                    ...
                    <p className="text-xs text-slate-400 mt-3">{char?.name || '角色'}正在活動中...</p>
                </div>
            )}
        </div>
    );
                ...
                <section className="px-5 pt-6 pb-5">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-slate-400 mt-1">角色獨立小紅書主頁</p>
                        </div>
                    </div>

                    <div className="flex items-end gap-8 mt-5">
                        <div><p className="text-lg font-bold text-slate-900">{ownedPosts.length}</p><p className="text-[10px] text-slate-400">發佈</p></div>
                        <div><p className="text-lg font-bold text-slate-900">{successfulReplies}</p><p className="text-[10px] text-slate-400">已回覆</p></div>
                    </div>

                    <p className="text-[11px] text-slate-500 leading-relaxed mt-4 border-l-2 border-rose-200 pl-3">
                        真實帳號可以與其他角色共用；這裡僅保存{char?.name || '該角色'}親自發布的 note_id。清除活動記錄不會刪除主頁歸屬。
                    </p>
                    <button
                    ...
                    >
                        {claimLoading ? '正在讀取真實帳號…' : '從真實帳號認領舊帖子 →'}
                    </button>
                </section>

                <div className="h-px bg-slate-100" />
                <div className="px-5 py-3 flex items-center justify-between">
                    <h3 className="text-xs font-bold text-slate-700">筆記</h3>
                    <span className="text-[10px] text-slate-400">按發佈時間排序</span>
                </div>

                {ownedPosts.length === 0 ? (
                    <div className="px-6 py-14 text-center">
                        <PencilSimple size={36} weight="thin" className="text-slate-300 mx-auto" />
                        <p className="text-sm font-medium text-slate-500 mt-3">還沒有發佈過筆記</p>
                        <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">下一次成功發帖後，會用唯一 note_id 自動出現在這裡。</p>
                    </div>
                ) : (
                        ...
                        {ownedPosts.map(post => (
                            ...
                            >
                                <div className="flex-1">
                                    <p className="text-sm font-bold text-slate-800 leading-snug line-clamp-3">{post.title || '無標題'}</p>
            ...
            {activities.length === 0 ? (
                <div className="flex flex-col items-center px-2 py-8 space-y-4">
                    <div className="text-center opacity-60">
                        ...
                        <p className="text-sm text-slate-500 font-medium mt-2">{char?.name || '角色'}還沒有自由活動記錄</p>
                    </div>

                    <div className="w-full bg-white/80 rounded-2xl border border-slate-100 p-4 space-y-3">
                        <p className="text-xs font-bold text-slate-600">自由活動是什麼？</p>
                        <p className="text-[11px] text-slate-500 leading-relaxed">
                            讓{char?.name || '角色'}自主使用小紅書 — 就像一個真實的人在刷手機。
                            ta會根據自己的性格和最近的聊天內容，決定要做什麼。
                        </p>
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-bold text-slate-400">ta可能會：</p>
                            <div className="grid grid-cols-2 gap-1.5">
                                {[
                                    { code: '270d', text: '發一條筆記' },
                                    { code: '1f50d', text: '搜感興趣的話題' },
                                    { code: '1f4f1', text: '刷首頁看看熱門' },
                                    { code: '1f3e0', text: '查看自己的主頁' },
                                    { code: '1f4ac', text: '回覆自己帖子的評論' },
                                    { code: '1f634', text: '或者什麼都不做' },
                                ].map((item, i) => (
                        ...
                        <div className="bg-rose-50 rounded-xl p-2.5">
                            <p className="text-[10px] text-rose-400 leading-relaxed">
                                活動結束後，{char?.name || '角色'}會記住看到的內容。下次聊天時，ta可能會主動跟你分享在小紅書上看到的有趣東西。
                            </p>
                        </div>
                    </div>

                    <p className="text-[10px] text-slate-300">點擊下方按鈕開始第一次自由活動</p>
                </div>
            ) : (
                activities.map(a => (
                    <button
        ...
        <div className="h-full flex flex-col bg-gradient-to-b from-rose-50 to-white">
                    ...
                    >
                        ...
                        <div>
                            <div className="flex items-center gap-1">
                                <h1 className="text-sm font-bold text-slate-800">{char?.name || '選擇角色'}</h1>
                                {!isRunning && (
                            ...
                            <p className="text-[10px] text-slate-400">{viewMode === 'profile' ? '角色主頁' : '活動記錄'}</p>
                        </div>
                    </button>
                </div>
                <div className="flex items-center gap-2">
                    {/* MCP status indicator */}
                    <div className={`w-2 h-2 rounded-full ${mcpStatus === 'connected' ? 'bg-emerald-400' : mcpStatus === 'error' ? 'bg-red-400' : 'bg-slate-300'}`} title={mcpStatus === 'connected' ? 'MCP已連接' : mcpStatus === 'error' ? 'MCP未連接' : '未檢測'} />
            ...
            {!mcpEnabled && (
                <div className="mx-4 mt-3 bg-amber-50 border border-amber-200 rounded-2xl p-3">
                    <p className="text-xs text-amber-700 font-medium">小紅書 MCP 未開啟</p>
                    <p className="text-[10px] text-amber-500 mt-1">
                        請前往 設置 → 實時感知 → 小紅書 MCP，開啟並配置 Server URL。
                    </p>
                </div>
            )}
