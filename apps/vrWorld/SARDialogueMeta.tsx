...
export function SARDialogueMeta({ speaker, npc, stars, replayTitle, onClose, closeLabel = '離開對話' }: {
    ...
    return <div className="srf-meta">
        <div className="srf-meta-content">
            <div className="srf-speaker">
                <b>{speaker}</b>
                {stars !== undefined && <span className={`srf-stars srf-stars-${npc}`} aria-label={`${SAR_NPC_NAMES[npc]} ${stars} 星，共五顆星`}>
                    {[1,2,3,4,5].map(n => <Star key={n} size={13} weight={n <= stars ? 'fill' : 'regular'}/>)}
                </span>}
            </div>
            {replayTitle && <span className="srf-replay-title" title={replayTitle}>{replayTitle}<small>回顧中</small></span>}
        </div>
