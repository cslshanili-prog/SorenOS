...
export function SARFamiliarityDialog({ npc, sceneId, onClose, onEditUserChibi, onSellFish }: {
    ...
    const name=npc==='caian'?'凱恩':'艾文';
    const run=async(action:()=>Promise<void>)=>{if(lock.current)return;lock.current=true;setBusy(true);setError('');try{await action();}catch(cause){if(mounted.current)setError(cause instanceof Error?cause.message:'沒有保存成功，請重試');}finally{lock.current=false;if(mounted.current)setBusy(false);}};
    const open=async()=>{
        if(sceneId){
            const current=readFishingMarketState(),past=familiarityProgress(current,npc).completed[sceneId],s=familiarityScene(sceneId);
            if(!s||s.npc!==npc||(!past&&!canPreviewFamiliarityEvent(s)))throw new Error('這段回憶還沒有解鎖');
        ...
        void deliverFamiliarityMessages().catch(()=>{if(mounted.current)setError('回憶已保存，私聊彩蛋待下次打開時重試');});
    };
    useEffect(()=>{
    ...
    const advance=(choice?:number,committedDraft=draft)=>void run(async()=>{
        ...
        void deliverFamiliarityMessages().catch(()=>setError('回憶已保存，私聊彩蛋待重試'));
    });
    ...
    const sentences=finished&&scene?.kind==='event'?[`結束${scene.rank}星事件：${scene.title}`]:sceneId&&finished?['這條分支看完了，可以返回選項，再試另一種回應。']:node?(line?dialogueSentences(line.text).map(sentence=>familiarityText(sentence,cursor?.userName||userName,cursor?.flags)):[node.effect?.title||' '])
    ...
    const text=serviceText??(sentences[pageIndex]||(market?' ':'正在走進活動室…'));
    ...
    const nextHint=serviceText!==null?'請選擇回應':busy?'保存中…':finished?(sceneId?'點擊返回名冊':canSellFish?'點擊繼續':'點擊返回活動室'):waitingForEffect&&lastSentence?'先完成上方操作':node&&lastLine&&lastSentence&&!node.next&&!node.choices?.length?'點擊收好這段回憶':visual&&visual.kind!=='confetti'&&lastSentence?'點擊收起，繼續對話':'點擊繼續';
    useEffect(()=>{
        const target=window as Window&{render_game_to_text?:()=>string;advanceTime?:(ms:number)=>void};
        const render=()=>JSON.stringify({mode:'sar-familiarity',npc,replay:!!sceneId,preview,stars:progress?.stars,scene:scene?.id,node:cursor?.nodeId,line:cursor?.line,sentence:pageIndex,text,choices:serviceText!==null?['賣魚','離開']:showChoices?node?.choices?.map(c=>familiarityText(c.label,userName,cursor?.flags)):[],effect:visual?.kind,confirmed:!!draft.confirmed,busy,error,finished,...(reviewEnabled&&sceneId?{reviewOpen,expressionEdits:edits.length,expressions:cast}:{})});
    ...
    return <section ref={root} tabIndex={-1} className={`srf-dialog srf-${npc} ${visual?'has-effect':''} ${reviewOpen?'is-reviewing':''}`} role="dialog" aria-modal="true" aria-label={`${name}${sceneId?'的回憶':'的日常'}`} onKeyDown={keyDown}>
        <SARDialogueBackdrop/>
        <div className="srf-body">
            ...
            <div className="srf-script">
                <SARDialogueMeta npc={npc} speaker={line?.speaker==='narrator'?'旁白':line?.speaker==='sully'?'Sully':line?.speaker==='caian'?'凱恩':line?.speaker==='aiven'?'艾文':name}
                    stars={progress?.stars||0} replayTitle={sceneId?scene?.title:undefined} onClose={sceneId?onClose:undefined}/>
                <button className="srf-bubble" type="button" aria-label="繼續對話" disabled={busy||!market||showChoices||serviceText!==null||(lastSentence&&waitingForEffect)} onClick={continueDialogue}>
                    ...
                    <span className="srf-next">{showChoices?(reviewOpen?'在校對欄選擇回應':'請選擇回應'):nextHint}<ArrowRight size={16}/></span>
                </button>
                {error&&<p className="srf-error" role="alert">{error}{!market&&<button disabled={busy} onClick={()=>void run(open)}>重新打開</button>}</p>}
            </div>
        </div>
        {serviceText!==null&&canSellFish&&<SARDialogueChoices><button type="button" onClick={onSellFish}>賣魚</button><button type="button" onClick={onClose}>離開</button></SARDialogueChoices>}
