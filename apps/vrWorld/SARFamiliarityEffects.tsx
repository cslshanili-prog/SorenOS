...
const BACKGROUNDS = [{ id: 'lounge', label: '沙發旁', position: '50% 49%' }, { id: 'desk', label: '管理員櫃檯', position: '50% 20%' }, { id: 'water', label: '水邊', position: '50% 94%' }];
...
function readPhoto(props: SARFamiliarityEffectsProps): PhotoDraft {
    ...
    const defaults: PhotoActor[] = [
        { id: 'caian', name: '凱恩', npc: 'caian', x: 26, y: 86, scale: 1 },
...
function PhotoStudio(props: SARFamiliarityEffectsProps) {
    ...
    const confirm = () => {
        ...
        onDraftChange({ ...latest.current.draft, date, confirmed: true, photo: { ...value, date }, caption: `Caian 笑得非常明顯。\nAiven 看著鏡頭，表情和平時沒有太大區別。\n${props.userName}也在照片裡。${friends ? '\n似乎朋友們也都在！' : ''}\n「沒有得到答案。不過是一次很好的會議！」` });
        setFlash(true);
    };
    ...
    return <div className={`srf-fx-photo ${confirmed ? 'is-confirmed' : ''}`}>
        <div className={`srf-fx-polaroid sar-artifact-photo ${back ? 'is-back' : ''}`}><span className="sar-photo-corner is-left" aria-hidden="true"/><span className="sar-photo-corner is-right" aria-hidden="true"/>
            <div className={`srf-fx-photo-stage ${flash ? 'is-flashing' : ''} ${back ? 'show-back' : ''}`} ref={stage} aria-label="合照構圖，橫向為 X，縱向為 Y">
                <div className={`srf-fx-photo-background ${selected === 'background' ? 'is-selected' : ''}`} style={{ cursor: confirmed ? 'default' : 'grab' }} onPointerDown={event => pointerDown(event, 'background')} onPointerMove={pointerMove} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }}>
                    <img src={roomArt} alt="SAR 活動室合照背景" draggable={false} style={{ objectPosition: backdrop.position, transform: `translate(${photo.panX}%,${photo.panY}%) scale(${photo.zoom})` }} />
                </div>
                {photo.actors.map(actor => <button key={actor.id} type="button" className={`srf-fx-photo-actor ${selected === actor.id && !confirmed ? 'is-selected' : ''}`} data-actor={actor.id} aria-label={`調整${actor.name}的位置`} disabled={confirmed}
...
function IdentityCard(props: SARFamiliarityEffectsProps) {
    ...
    return <div className="srf-fx-id-wrap sar-artifact-id-stage"><article className={`srf-fx-id sar-artifact-identity ${membership ? 'is-member' : 'is-admin'}`}>
        <div className="sar-id-security" aria-hidden="true"/><header><div><strong>SAR</strong><span>彼方活動室</span></div><small>{membership ? 'MEMBER CARD' : 'ADMINISTRATOR'}</small></header>
        <div className="srf-fx-id-main sar-id-main"><div className="srf-fx-id-portrait">{membership ? <Person actor={{ name, chibi: (props.replay || props.draft.confirmed) && member ? member.chibi : props.userChibi }}/> : <SARPortrait who="caian" expression="normal"/>}<span className="sar-id-photo-label">{membership ? 'FIRST VISITOR' : 'CAIAN'}</span></div>
            <div className="sar-id-person"><small>{membership ? '成員姓名 / NAME' : '管理員 / NAME'}</small><strong>{name}</strong><span className="sar-id-number">No. <b>{membership ? '0001' : '0000'}</b></span><p>{membership ? '正式上任後的第一位訪客' : '彼方兼職管理員'}</p></div>
        </div>
        <div className="sar-id-signature"><span>記錄人 <b>Caian</b></span><SARArtifactSeal/><small>{membership ? 'WELCOME TO SAR' : 'OFFICIAL RECORD'}</small></div>
        <footer><span>SAR / {membership ? 'MEMBERSHIP' : 'STAFF'}</span><span>{dateLabel(props.draft)}</span></footer>
    </article><p className="srf-fx-artifact-note">{membership ? '他為第一位訪客，認真留了一個編號。' : '照片是系統拍的。本人比這個精神多了。'}</p>
        {membership && !props.replay && <div className="srf-fx-id-actions">{props.onEditUserChibi && <button type="button" onClick={props.onEditUserChibi}><User size={16}/>調整我的形象</button>}<button type="button" className="srf-fx-primary" onClick={() => props.onDraftChange({ ...props.draft, date: new Date().toLocaleDateString('zh-CN'), confirmed: true, membership: { name: props.userName, chibi: props.userChibi || '' } })}>{props.draft.confirmed ? <><Check size={16}/>形象已確認</> : '就用這個形象'}</button></div>}
    </div>;
}
...
function MeetingRecord(props: SARFamiliarityEffectsProps) {
    return <div className="sar-document-stage"><article className="srf-fx-meeting sar-artifact-document"><i className="sar-document-clip" aria-hidden="true"/>
        <header><span><SARArtifactSeal/><small>SAR ACTIVITY SPACE<br/>會議檔案 / MEETING RECORD</small></span><strong>001</strong></header>
        <div className="sar-document-title"><small>MINUTES OF THE FIRST MEETING</small><h3>第一次正式討論會</h3><span>{dateLabel(props.draft)}</span></div>
        <dl><dt><i>01</i>議題</dt><dd className="sar-document-topic">人工人格的沉默是否能夠<br className="sar-document-break"/>被視為一種自主選擇</dd><dt><i>02</i>參與者</dt><dd>Caian · Aiven<span>（列席）</span> · {props.userName}</dd><dt><i>03</i>結論</dt><dd className="srf-fx-meeting-conclusion">{String(props.flags.meetingConclusion || '未得出結論')}</dd></dl>
        <div className="sar-document-postscript"><small>記錄人備註</small><p>討論仍然有意義。</p><span>Caian</span></div>
...
function Chimera({ onReady }: { onReady?: () => void }) {
    ...
    return <div className="srf-fx-chimera-model" data-model-ready={ready}><div ref={host} className="srf-fx-chimera-view" />{(!ready || failed) && <div className="srf-fx-chimera-fallback"><DinoIcon species="aiven-chimera" color="#b69bbb" /><small>{failed ? '模型暫時未加載' : '有什麼被釣上來了……'}</small></div>}<span className="srf-fx-chimera-question">？？？</span></div>;
}

function ScatteredProps({ items }: { items: string[] }) {
    return <div className="srf-fx-loot" aria-label="剛剛釣上來的東西">{items.map((item, i) => <div className="srf-fx-loot-item" key={`${item}:${i}`} style={{ '--i': i, '--tilt': `${[-6, 5, -2, 7, -4, 3, -5][i % 7]}deg` } as React.CSSProperties}>
        <span className="srf-fx-loot-art">{/[鱼魚]/.test(item) ? <FishArt speciesId="cloud-carp" size={74} /> : /恐[龙龍]/.test(item) ? <DinoIcon species="triceratops" color="#b5a0be" /> : /券/.test(item) ? <Ticket size={49} weight="duotone" /> : /胸牌/.test(item) ? <IdentificationCard size={47} weight="duotone" /> : /存[档檔]卡/.test(item) ? <FloppyDisk size={46} weight="duotone" /> : /雨靴/.test(item) ? <svg viewBox="0 0 70 65" role="img" aria-label="雨靴"><path d="M18 5h30l-3 32 15 9c6 4 5 14-1 14H12c-4-2-4-7-1-12l8-12z" fill="#bba789" stroke="#756552" strokeWidth="2" /><path d="M11 54h48M19 10h27" stroke="#f2e6cd" strokeWidth="4" /></svg> : <Cpu size={47} weight="duotone" />}</span><small>{item}</small>
    </div>)}</div>;
}
...
export function SARFamiliarityEffects(props: SARFamiliarityEffectsProps) {
    ...
    const title = copy(effect.title, props) || '物品詳情';
    ...
    return <section className={`srf-fx srf-fx-${effect.kind} ${inspectable ? 'is-inspectable' : ''}`} aria-label={copy(effect.title, props) || '特別演出'} onClick={event => {
        ...
        {effect.kind === 'photo-studio' ? <PhotoStudio {...props} />
            ...
            : effect.kind === 'chimera' ? <div className="srf-fx-chimera-wrap"><Chimera /><h3>{copy(effect.title, props)}</h3><p>{copy(effect.text, props)}</p><small>拖動看看它的另一面</small></div>
            : effect.kind === 'loot-burst' ? <div className="srf-fx-loot-wrap"><small>FROM THE WATER</small><h3>{copy(effect.title, props)}</h3><div className="srf-fx-loot-scene"><div className="srf-fx-loot-npc is-caian"><SARNpcChibi who="caian" /><small>凱恩</small></div><ScatteredProps items={(effect.items || []).map(item => copy(item, props))} /><div className="srf-fx-loot-npc is-aiven"><SARNpcChibi who="aiven" /><small>艾文</small></div></div></div>
            : effect.kind === 'discount' ? <div className="srf-fx-discount-stage"><div className="srf-fx-discount-ticket"><small>SAR / MODULE SHOP</small><strong>{discountLabel(effect)}<em>折</em></strong><p>{copy(effect.title, props)}</p><span>{copy(effect.text, props) || '模塊商店限時優惠'}</span><i /><i /></div></div>
            : effect.kind === 'mystery-button' ? <div className="srf-fx-mystery"><small>SAR / UNKNOWN DEVICE</small><button type="button" className={confirmed ? 'is-pressed' : ''} aria-label="按下神秘按鈕" onClick={() => onDraftChange({ ...draft, confirmed: true, mysteryPressed: true })}>{confirmed ? <Check size={48} /> : <Question size={48} weight="bold" />}</button><h3>{copy(effect.title, props) || '一個意義不明的按鈕'}</h3><p>{confirmed ? '咔噠。' : copy(effect.text, props) || '似乎可以按下去。'}</p></div>
        ...
        {effect.interactive && !ownConfirmation && !props.replay && <button type="button" className="srf-fx-primary srf-fx-confirm" onClick={confirm}>{confirmed ? <><Check size={16} />看清了</> : effect.kind === 'chimera' ? '仔細看過了' : '繼續看看'}</button>}
    </section>;
}
