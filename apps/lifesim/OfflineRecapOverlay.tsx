        ...
        <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
            <div className="bg-gray-900 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-white/10 max-h-[85vh] flex flex-col">
                ...
                <div className="bg-gradient-to-b from-indigo-900 to-purple-950 px-5 pt-6 pb-4 text-center flex-shrink-0">
                    <p className="text-purple-300/60 text-[10px] tracking-widest mb-1">都市快報 · 號外</p>
                    <p className="text-purple-100 text-xl font-bold">你不在的時候...</p>
                    <p className="text-purple-300/70 text-xs mt-1">{elapsedDesc}過去了</p>
                </div>

                {/* 事件列表 */}
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
                    {recap.length === 0 ? (
                        <p className="text-white/40 text-center text-sm py-6">風平浪靜，什麼都沒發生。</p>
                    ...
                    {recap.length > 20 && (
                        <p className="text-white/30 text-center text-[10px]">...還有{recap.length - 20}件事沒來得及報道</p>
                    )}
                </div>
                    ...
                    <div className="flex justify-around text-center mb-3">
                        {fightCount > 0 && (
                            <div>
                                ...
                                <p className="text-white/40 text-[9px]">場衝突</p>
                            </div>
                        )}
                        {romanceCount > 0 && (
                            <div>
                                <p className="text-pink-400 text-lg font-bold">{romanceCount}</p>
                                <p className="text-white/40 text-[9px]">段戀情</p>
                            </div>
                        )}
                        <div>
