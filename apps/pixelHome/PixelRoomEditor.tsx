...
// 編輯器放大倍率（用整數避免子像素渲染問題）
const EDITOR_SCALE = 1.5;
const SNAP_SUBDIVISIONS = 3; // 每格細分3段，拖拽更精細

/** 吸附到細分格子。允許中心點略微溢出房間邊界（-30..130），這樣
 *  大傢俱也能把視覺底邊 / 邊緣放到房間邊上，而不是停在"中心點貼邊"。 */
...
const PixelRoomEditor: React.FC<Props> = ({ charId, charName, charSprite, userName, roomId, layout, assets, onUpdate, onOpenLibrary }) => {
  ...
  const [floorTileSize, setFloorTileSize] = useState(TILE); // 地磚平鋪大小（可調）
  // 鋪設模式 + 偏移
  ...
  const [charStep, setCharStep] = useState(0); // 走路幀 0/1
  ...
  const dragConfirmedRef = useRef(false); // 是否已超過拖拽閾值
  ...
  // 多指觸控狀態（pinch-to-zoom）
  const touchStateRef = useRef<{
    active: boolean;        // 是否正在雙指操作
    initialDist: number;    // 初始雙指距離
    initialZoom: number;    // 初始縮放值
  }>({ active: false, initialDist: 0, initialZoom: 1 });

  const DRAG_THRESHOLD = 8; // 像素，超過才算拖拽

  // 碰撞檢測：緩存每個資產的 alpha 遮罩
  ...
  // 像素走路：每 600ms 走一格，走 2-3 步就停，停 4-8 秒再動
  useEffect(() => {
    const pickTarget = () => {
      // 只走附近 2-3 格，不橫穿整個房間
  ...
  useEffect(() => {
    ...
    const build = async () => {
      ...
      // 安全轉送：傢俱重排後，如果角色當前正好被壓在傢俱底下（卡住），
      // 用 BFS 找最近的一個地面空格把它送過去，避免卡死。
      const cur = charPosRef.current;
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent, slotId: string) => {
    ...
    dragConfirmedRef.current = false; // 還沒超過閾值
  ...
  const clearAllFurniture = useCallback(() => {
    ...
    if (total === 0) return;
    if (!window.confirm(`確定清空這個房間裡的 ${total} 件傢俱嗎？（自由放置的 ${userCount} 件會被刪除，默認槽位的 ${filledDefaults} 件會恢復為空）`)) return;

    setFurniture(prev => {
      const next = prev
        .filter(f => f.isDefault !== false)        // 扔掉用戶自由放置的
        .map(f => ({ ...f, assetId: null }));      // 默認槽位清空素材
      saveLayout(next);
      return next;
    });
    setSelectedSlot(null);
      ...
      <div ref={outerRef} className="flex-1 overflow-hidden flex items-center justify-center"
        ...
        <div style={{ transform: `scale(${zoom})`, transformOrigin: 'center center', transition: touchStateRef.current.active ? 'none' : 'transform 0.1s ease-out' }}>
          <div ref={stageRef} className="relative select-none overflow-visible" style={{ width: roomPxW, height: roomPxH }}>
            ...
            {/* 格子網格：只有正在拖動 / 選中某件傢俱時才顯示，避免平時看到滿屏白十字 */}
            ...
            {[...furniture].sort((a, b) => {
              ...
              // z-index 按傢俱中心點 f.y 排（俯視視角里 y 越大越靠前）。
              // 之前用"視覺底邊"(f.y + halfHPct) 排序，對掛在牆上的大件傢俱會嚴重虛高：
              // 它們的視覺底邊會向下延伸、甚至超過角色腳下，z 就比角色還大——
              // 結果是牆上的烤箱/儲物櫃擋住了路過角色的頭。回到中心點 y 更符合直覺。
              // "背後大傢俱應該壓住前面小傢俱"的場景由用戶手動"置頂"解決。
              //
              // 分桶策略（必須全部為正整數，否則負 z-index 會沉到牆壁/地板背後）：
              //   rug:   1
              //   back:  2..5          （比普通傢俱低，但仍在地板/牆之上）
              //   auto:  20..~420      （按中心 y 遞增）
              //   front: 1000..~1400   （永遠壓住 auto 傢俱）
              //   selected: 2000       （操作焦點）
              ...
              return (
                <div key={f.slotId} style={{
                  ...
                  left: posX,
                  top: posY,
                  // 顯式釘死容器寬度為整數像素（高度隨圖自適配，保留原有縱橫比），
                  // 避免 img 默認 inline 的基線行間隙 + 亞像素舍入讓"越往右看起來越大/越小"
                  width: furSize,
                  zIndex: zIdx,
            ...
            {/* 角色小人（像素步行）
               z-index 和傢俱共享一套座標：錨點是腳底（translate -100%），視覺底邊 = charPos.y，
               所以直接用 Math.round(charPos.y * 4) + 20 和傢俱的 autoZ 保持同一尺度，
               否則傢俱的 autoZ（~20..420）永遠遠大於原來的 charPos.y+2，角色就會被壓在傢俱底下。

               寬高顯式釘死為整數像素（40×40），避免 inline img + h-auto 在不同 x 位置上
               因亞像素舍入讓角色"走到右邊變小"。 */}
            {charSprite && (
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex gap-1">
            <ModeBtn label="瀏覽" active={mode === 'view'} onClick={() => { setMode('view'); setSelectedSlot(null); trackEvent('切换像素房间装修模式', { mode: 'view' }); }} />
            <ModeBtn label="編輯" active={mode === 'edit'} onClick={() => { setMode('edit'); trackEvent('切换像素房间装修模式', { mode: 'edit' }); }} />
            <ModeBtn label="記憶" active={showMemory} onClick={() => setShowMemory(!showMemory)} />
          </div>
          <div className="flex gap-1 flex-wrap justify-end items-center">
            <ToolBtn label="放傢俱" color="bg-green-700" onClick={() => onOpenLibrary('__add__')} />
            <ToolBtn label="牆紙" color="bg-violet-700" onClick={() => wallInputRef.current?.click()} />
            {/* 純色牆：label 包 input[type=color] 做成按鈕 */}
            {(() => {
              const d = decodeColorField(wallColor);
            ...
            {customWall && <ToolBtn label="×牆" color="bg-violet-900" onClick={() => resetTexture('wall')} />}
            <ToolBtn label="地磚" color="bg-amber-800" onClick={() => floorInputRef.current?.click()} />
            {(() => {
              const d = decodeColorField(floorColor);
          ...
          <div className="p-2.5 bg-slate-700/60 rounded-xl space-y-2 mb-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-200 font-bold">
                {texturePreview.target === 'wall' ? '牆紙預覽' : '地磚預覽'}
              </span>
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1 text-center">
                ...
                <span className="text-[9px] text-slate-400">原圖直接用</span>
              </div>
            </div>
            <div className="flex gap-1">
        ...
        {selectedFurniture && (
          <div className="p-2.5 bg-slate-700/60 rounded-xl space-y-2 mb-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-200 font-bold">
                {selectedSlotDef?.name || (selectedFurniture.assetId ? assets.find(a => a.id === selectedFurniture.assetId)?.name : '傢俱')}
              </span>
            ...
            <SliderRow label="旋轉" min={-180} max={180} step={15} value={selectedFurniture.rotation}
              onChange={v => updateFurniture(selectedSlot!, { rotation: v })} display={`${selectedFurniture.rotation}°`} />
            {/* 前後遮擋手動覆蓋 */}
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-400 w-8">遮擋</span>
              <div className="flex-1 flex gap-1">
                ...
                <ZOrderBtn label="自動" active={!selectedFurniture.zOrder || selectedFurniture.zOrder === 'auto'}
                  onClick={() => updateFurniture(selectedSlot!, { zOrder: 'auto' })} />
                <ZOrderBtn label="置頂" active={selectedFurniture.zOrder === 'front'}
            ...
            <div className="flex gap-2">
              <button onClick={() => onOpenLibrary(selectedSlot)}
                className="flex-1 py-1.5 bg-amber-600 text-white text-[10px] font-bold rounded-lg active:scale-95">替換素材</button>
              {selectedFurniture.isDefault === false && (
                <button onClick={() => deleteFurniture(selectedSlot!)}
                  className="px-3 py-1.5 bg-red-600 text-white text-[10px] font-bold rounded-lg active:scale-95">刪除</button>
              )}
              {selectedFurniture.assetId && selectedFurniture.isDefault !== false && (
                <button onClick={() => updateFurniture(selectedSlot!, { assetId: null })}
                  className="px-3 py-1.5 bg-slate-600 text-slate-200 text-[10px] font-bold rounded-lg active:scale-95">還原</button>
              )}
            </div>
          </div>
        )}
          ...
          <div className="p-2.5 bg-slate-900/80 rounded-xl border border-slate-700/50">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">
                {roomDisplayName}的記憶 ({memories.length})
              </span>
              <button onClick={loadMemories} className="text-[9px] text-slate-400 hover:text-slate-200">刷新</button>
            </div>
            {memoryLoading ? (
              <div className="text-center py-4 text-slate-500 text-xs">加載中...</div>
            ) : memories.length === 0 ? (
              <div className="text-center py-4 text-slate-500 text-xs">暫無記憶</div>
            ) : (
