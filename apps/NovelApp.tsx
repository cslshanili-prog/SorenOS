...
const NovelApp: React.FC = () => {
    ...
    const [collabGroupId, setCollabGroupId] = useState(GROUP_FILTER_ALL); // 共創者選擇的分組篩選
    ...
    const [libraryGroupId, setLibraryGroupId] = useState(GROUP_FILTER_ALL); // 角色庫的分組篩選
    const [isEditingPersona, setIsEditingPersona] = useState(false);
    const [personaDraft, setPersonaDraft] = useState('');

    // 彈窗展示期間角色可能被 updateCharacter 更新（如保存檔案後），始終從 characters 取最新數據
    ...
    const savePersonaDraft = () => {
        if (!libraryChar) return;
        if (!personaDraft.trim()) { addToast('檔案內容不能為空', 'error'); return; }
    ...
    const handleCreateBook = () => {
        if (!tempTitle.trim()) { addToast('請輸入標題', 'error'); return; }
    ...
    const handleDeleteBook = async (id: string) => {
        setConfirmDialog({
            isOpen: true, title: '刪除作品', message: '確定要刪除這本小說嗎？此操作無法撤銷。', variant: 'danger',
            onConfirm: () => { deleteNovel(id); if (activeBook?.id === id) setView('shelf'); addToast('已刪除', 'success'); setConfirmDialog(null); trackEvent('删除一本书稿'); }
        });
    };
    ...
    const saveProtagonist = () => {
        if (!editingProtagonist || !editingProtagonist.name.trim()) { addToast('角色名不能為空', 'error'); return; }
    ...
    const importWorldbook = (wb: any) => {
        ...
        addToast(`已導入設定: ${wb.title}`, 'success');
    ...
    const ProtagonistCard = ({ p, onDelete, onClick }: { p: NovelProtagonist, onDelete?: () => void, onClick?: () => void }) => (
        <div onClick={onClick} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm relative group cursor-pointer hover:border-slate-400 transition-colors">
            ...
            <div className="text-xs text-slate-500 mt-1 line-clamp-2">{p.description || "暫無描述"}</div>
            ...
            <div className="h-full w-full bg-slate-50 flex flex-col font-sans">
                <div className="bg-white/80 backdrop-blur-md border-b border-slate-200 shrink-0 sticky top-0 z-20" style={{ paddingTop: 'var(--chrome-top)' }}>
                    ...
                    <div className="flex justify-between items-center w-full">
                        ...
                        <span className="font-bold text-slate-800 text-lg tracking-wide">角色庫</span>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                    <section>
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Robot size={14} /> 系統角色 (AI Collaborators)</h3>
                        {/* 分組篩選（沒建分組時不渲染） */}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {filterCharactersByGroup(characters, characterGroups, libraryGroupId).map(c => (
                                <div key={c.id} onClick={() => { setLibraryPersonaChar(c); setShowPersonaModal(true); }} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center gap-3 cursor-pointer hover:shadow-md transition-all active:scale-95">
                                    ...
                                    <div className="text-center"><div className="font-bold text-slate-700 text-sm">{c.name}</div><div className="text-[10px] text-slate-400 mt-1 px-2 py-0.5 bg-slate-50 rounded-full">共創者</div></div>
                                </div>
                            ))}
                        </div>
                    </section>
                    <section>
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><MaskHappy size={14} /> 歷史劇中人 (From History)</h3>
                        {historyProtagonists.length === 0 ? <div className="text-center py-8 text-slate-400 text-xs">暫無歷史角色數據</div> : <div className="grid grid-cols-1 gap-3">{historyProtagonists.map((p, idx) => (<div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm"><div className="flex justify-between items-start mb-2"><span className="font-bold text-slate-800">{p.name}</span><span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded border border-indigo-100">{p.role}</span></div><p className="text-xs text-slate-500 leading-relaxed line-clamp-3">{p.description || "暫無描述"}</p></div>))}</div>}
                    </section>
                </div>

                <Modal isOpen={showPersonaModal} title={libraryChar?.name || '角色風格'} onClose={closePersonaModal} footer={
                    isEditingPersona ? (<>
                        ...
                        <button onClick={startEditPersona} className="flex-1 py-3 bg-indigo-50 text-indigo-600 font-bold rounded-2xl active:scale-95 transition-transform">編輯檔案</button>
                        <button onClick={closePersonaModal} className="flex-1 py-3 bg-slate-100 text-slate-500 font-bold rounded-2xl active:scale-95 transition-transform">關閉</button>
                    </>)
                }>
                        ...
                        {libraryChar ? (
                            isEditingPersona ? (
                                <div className="space-y-2">
                                    ...
                                    <button onClick={() => { setPersonaDraft(analyzeWriterPersonaSimple(libraryChar)); trackEvent('重置创作档案为自动分析'); }} className="text-xs text-slate-400 underline">重置為自動分析</button>
                                </div>
                            ) : (
        ...
        return (
            <div className="h-full w-full bg-slate-50 flex flex-col font-sans relative">
                ...
                <div className="p-6 grid grid-cols-2 gap-5 overflow-y-auto pb-24">
                        ...
                        return (
                            <div key={book.id} onClick={() => { setActiveBook(book); setView('write'); trackEvent('打开书稿写作页'); }} className="group relative aspect-auto min-h-[14rem] bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-100 cursor-pointer flex flex-col">
                                ...
                                <div className="p-4 flex-1 flex flex-col justify-between">
                                    <p className="text-xs text-slate-500 line-clamp-3 leading-relaxed mb-3">{book.summary || '暫無簡介...'}</p>
                    ...
                    {novels.length === 0 && <div className="col-span-2 flex flex-col items-center justify-center h-64 text-slate-300 gap-3"><PenNib size={48} className="opacity-50" /><span className="text-sm font-sans">點擊右上角，開始創作</span></div>}
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="h-full w-full bg-slate-50 flex flex-col font-sans relative">
                ...
                <div className="bg-white border-b border-slate-200 shrink-0 sticky top-0 z-20" style={{ paddingTop: 'var(--chrome-top)' }}>
                    ...
                    <span className="font-bold text-slate-800">{view === 'create' ? '新建書稿' : '小說設定'}</span>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-20">
                    <section className="space-y-4">
                        <input value={tempTitle} onChange={e => setTempTitle(e.target.value)} placeholder="書名" className="w-full text-2xl font-bold bg-transparent border-b border-slate-200 py-2 outline-none focus:border-slate-800 font-serif" />
                        <input value={tempSubtitle} onChange={e => setTempSubtitle(e.target.value)} placeholder="卷名/副標題" className="w-full text-sm font-bold bg-transparent border-b border-slate-200 py-2 outline-none focus:border-slate-800 text-slate-600" />
                        <textarea value={tempSummary} onChange={e => setTempSummary(e.target.value)} placeholder="一句話簡介..." className="w-full h-20 bg-slate-100 rounded-xl p-3 text-sm resize-none outline-none" />
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">內頁風格</label>
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">自定義封面</label>
                            <div className="flex gap-3 items-center">
                                ...
                                <div className="flex-1 space-y-2"><input value={coverInputUrl} onChange={e => setCoverInputUrl(e.target.value)} onBlur={handleCoverUrlBlur} placeholder="粘貼圖片鏈接..." className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-400" />{tempCoverImage && <button onClick={() => { setTempCoverImage(''); setCoverInputUrl(''); }} className="text-xs text-red-400 underline">清除封面</button>}</div>
                            </div>
                        </div>
                    </section>
                    <section className="space-y-4">
                        <div className="flex justify-between items-center"><label className="text-xs font-bold text-slate-400 uppercase block">世界觀設定</label><button onClick={() => { setIsWorldbookModalOpen(true); trackEvent('打开导入世界书弹窗'); }} className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold hover:bg-indigo-100 flex items-center gap-1"><Books size={12} /> 導入世界書</button></div>
                        <textarea value={tempWorld} onChange={e => setTempWorld(e.target.value)} placeholder="世界觀設定..." className="w-full h-32 bg-white border border-slate-200 rounded-xl p-3 text-sm resize-none outline-none focus:border-slate-400" />
                    </section>
                    <section className="space-y-4">
                        <label className="text-xs font-bold text-slate-400 uppercase block">共創者</label>
                        {/* 分組篩選（沒建分組時不渲染）：只影響可選項的顯示，不影響已勾選共創者 */}
                ...
                <Modal isOpen={isProtagonistModalOpen} title="編輯角色" onClose={() => setIsProtagonistModalOpen(false)} footer={<button onClick={saveProtagonist} className="w-full py-3 bg-slate-800 text-white font-bold rounded-2xl">保存</button>}>{editingProtagonist && (<div className="space-y-4"><div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">姓名</label><input value={editingProtagonist.name} onChange={e => setEditingProtagonist({...editingProtagonist, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold" /></div><div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">定位</label><input value={editingProtagonist.role} onChange={e => setEditingProtagonist({...editingProtagonist, role: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm" placeholder="主角 / 反派" /></div><div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">設定</label><textarea value={editingProtagonist.description} onChange={e => setEditingProtagonist({...editingProtagonist, description: e.target.value})} className="w-full h-32 bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm resize-none leading-relaxed" /></div></div>)}</Modal>
                <Modal isOpen={isProtoImportOpen} title="導入角色" onClose={() => setIsProtoImportOpen(false)}><div className="flex p-1 bg-slate-100 rounded-xl mb-3"><button onClick={() => setImportTab('system')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${importTab === 'system' ? 'bg-white shadow text-indigo-600' : 'text-slate-400'}`}>系統角色 (AI)</button><button onClick={() => setImportTab('history')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${importTab === 'history' ? 'bg-white shadow text-indigo-600' : 'text-slate-400'}`}>歷史角色</button></div><div className="max-h-[50vh] overflow-y-auto no-scrollbar space-y-3 p-1">{importTab === 'system' && characters.map(c => (<button key={c.id} onClick={() => handleImportProtagonist({name: c.name, role: '客串', description: c.description})} className="w-full flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl hover:border-indigo-200 shadow-sm active:scale-95 transition-all text-left"><TokenImg value={c.avatar} className="w-8 h-8 rounded-full object-cover" /><div className="flex-1 min-w-0"><div className="font-bold text-sm text-slate-700">{c.name}</div><div className="text-[10px] text-slate-400 truncate">{c.description}</div></div></button>))}{importTab === 'history' && historyProtagonists.map((p, idx) => (<button key={`hist-${idx}`} onClick={() => handleImportProtagonist(p)} className="w-full flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl hover:border-indigo-200 shadow-sm active:scale-95 transition-all text-left"><div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500 border border-slate-200">{p.name[0]}</div><div className="flex-1 min-w-0"><div className="font-bold text-sm text-slate-700">{p.name}</div><div className="text-[10px] text-slate-400 truncate">{p.role} - {p.description || "無描述"}</div></div></button>))}</div></Modal>
                <Modal isOpen={isWorldbookModalOpen} title="導入世界書設定" onClose={() => setIsWorldbookModalOpen(false)}><div className="max-h-[50vh] overflow-y-auto no-scrollbar space-y-2 p-1">{worldbooks.map(wb => (<button key={wb.id} onClick={() => importWorldbook(wb)} className="w-full text-left p-3 rounded-xl border border-slate-100 hover:border-indigo-300 bg-white shadow-sm active:scale-95 transition-all"><div className="font-bold text-slate-700 text-sm">{wb.title}</div><div className="text-[10px] text-slate-400 mt-1">{wb.category || '未分類'}</div></button>))}</div></Modal>
            </div>
        );
    }
