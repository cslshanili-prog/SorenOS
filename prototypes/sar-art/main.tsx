...
const visitors=[['sample-reader','示例訪客一','cabinet',caian],['sample-fisher','示例釣客','fishing',aiven],['sample-player','示例訪客二','garden',caian]].map(([id,name,sarActivity,img])=>({id,name,vrState:{enabled:true,currentRoom:'sar',sarActivity,chibi:{img,scale:1,offsetY:0,flip:false}}} as CharacterProfile));
function Preview(){
    ...
    return <>
        ...
        <header style={{position:'absolute',top:12,left:16,pointerEvents:'none',color:'#78654a'}}><small style={{fontSize:8}}>美術聯調 · 含示例訪客</small><h1 style={{fontSize:16,margin:0}}>SAR 活動室</h1></header>
        <div style={{position:'absolute',bottom:0,left:0,right:0,display:'flex',justifyContent:'center',gap:12,fontSize:10,color:'#77684f',background:'#fffdf4cc'}}><button onClick={()=>setNpcs(v=>!v)} style={{minHeight:30}}>NPC {npcs?'顯示':'隱藏'}</button><output data-testid="last-action">{action}</output><a href="?edit=portraits">調整立繪</a><a href="/test/fixtures/kanata.html?npcs=show">進入完整彼方</a></div>
