...
try{
  await page.goto('http://127.0.0.1:5182/test/fixtures/kanata.html',{waitUntil:'domcontentloaded',timeout:60000});await page.getByRole('button',{name:'下一頁房間',exact:true}).click({timeout:60000});
  const entry=page.getByRole('button',{name:'進入恐龍箱庭',exact:true});await entry.waitFor();await page.waitForTimeout(300);
  ...
  await page.getByRole('button',{name:'返回 SAR 活動室'}).click();await page.getByRole('button',{name:'進入水域',exact:true}).click();await page.getByRole('button',{name:'圖鑑',exact:true}).click();
  await page.getByRole('button',{name:'帶橡皮泥恐龍去箱庭 →',exact:true}).waitFor();assert.ok(await page.getByRole('img',{name:'霸王龍橡皮泥模型'}).count());
  await page.screenshot({path:`${out}/fishing-collection.png`});await page.getByRole('button',{name:'帶橡皮泥恐龍去箱庭 →',exact:true}).click();await page.waitForSelector('.clay-app[data-ready="true"]');
  ...
  await page.getByRole('button',{name:'返回 SAR 活動室'}).click();await page.setViewportSize({width:320,height:740});await page.screenshot({path:`${out}/sar-320.png`});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth),320);
