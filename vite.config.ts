...
// 構建時抓 git 分支 + short commit + UTC+8 構建時間，注入到版本信息顯示。
// 非 git 環境（容器、tarball 部署）退化成 'unknown'，不影響構建。
//
// 顯示規則：
//   - 默認在 main / master 上隱藏（視為正式發佈），其他分支顯示
//   - CI detached HEAD 優先讀 GITHUB_REF_NAME / VERCEL_GIT_COMMIT_REF / CF_PAGES_BRANCH / BRANCH(Netlify)
//   - VITE_HIDE_BUILD_BADGE=1 強制隱藏（覆蓋默認）
//   - VITE_SHOW_BUILD_BADGE=1 強制顯示（在 master 本地調試用）
...
export default defineConfig({
  ...
  // GitHub Pages 發佈時使用相對路徑，避免倉庫子路徑導致資源 404
  base: process.env.GITHUB_PAGES ? './' : '/',
  esbuild: {
    // 只剝 debugger，保留 console.* —— 部署後按 F12 仍能看到運行時日誌，方便排查。
    drop: ['debugger'],
  },
  server: {
    proxy: {
      '/api/minimax/t2a': {
        ...
        secure: true,
        rewrite: () => '/v1/t2a_v2',
        // Route to 國服 / 海外 based on X-MiniMax-Region header sent by the client.
        router: (req) => {
      ...
      // 魚聲 Fish Audio TTS：轉發到 https://api.fish.audio/v1/tts（返回二進制音頻）
      '/api/fishaudio/tts': {
      ...
      // ElevenLabs TTS：開發環境把同源查詢參數改寫到官方 voice_id 路徑。
      '/api/elevenlabs/tts': {
  ...
  build: {
    ...
    rollupOptions: {
      // 關鍵修復：將這些包排除在打包之外，讓瀏覽器通過 index.html 的 importmap 加載
      external: ['katex'],
      ...
      output: {
        manualChunks(id) {
          if (id.includes('node_modules')) {
            ...
            // VRM/Three 只在懶加載的 CallApp 視頻模式使用。單獨成包，避免 3D 引擎
            // 被通用 vendor 首屏加載，普通聊天/桌面用戶無需支付這部分體積。
