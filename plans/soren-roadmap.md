...
| 4 | 聊天設置改全螢幕 | 進行中（第一批：頁面＋Relationship，#24；第二批：已讀不回，#25；第三批：延遲自動回覆，#26；雲端延遲回覆，#27） |
...
- 本地與雲端去重：
  ...
  - 雲端過點三分鐘還沒收到回覆 → 問雲端任務狀態：還在排隊就等，行沒了算已經發過（推播或補收會送到），失敗了才由本地補回。角色的 2.0 被關掉（「取消全部」會一起掐掉）時也由本地補。
