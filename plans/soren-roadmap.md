...
| 4 | 聊天設置改全螢幕 | 完成（第一批：頁面＋Relationship，#24；第二批：已讀不回，#25；第三批：延遲自動回覆，#26；雲端延遲回覆，#27；第四批：線下邀請＋動作描寫，#29；第五批：角色主動打電話，#30；最後一批雙向拉黑＋臨時會話：設計見 [`plans/block-temp-chat-design.md`](./block-temp-chat-design.md)，拉黑本身 #42、拉黑中見面 #43，臨時會話 #44） |
...
- **精簡人設**（`utils/briefPersona.ts`，有單測）：神經鏈接編輯頁「世界觀」下面一格，100～200 字、寫「別人眼中的 TA」：身份、性格、說話方式、話量、熟人／生人差別，不寫跟用戶的私密關係和劇情。可手寫，也可按「AI 生成」用全局 API 從完整人設壓縮；生成時記下人設指紋（`briefPersonaSource`），之後人設改了就提示重新生成，手寫或手改過就不再提示。用在「別人要知道 TA 是怎樣的人」的地方：朋友圈批次留言替 TA 抓口吻（沒填退回從完整人設挑講個性的句子，`utils/momentsVoice.ts`）、查手機的真實角色名單、家園生成 NPC 時的成員介紹。角色自己的私聊照舊用完整人設。起因是高冷角色在朋友圈連環「哈哈哈」。
