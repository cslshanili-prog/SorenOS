...
const close=async()=>{if(await button('離開對話').count())await button('離開對話').click();};
const node=async(id,nodeId,line=0)=>{
    ...
    await page.evaluate(async({id,nodeId,line})=>{
        ...
        state.sarFamiliarity.npcs[scene.npc].pending={runId:id+':ui',sceneId:id,nodeId:nodeId||scene.start,line,revision:0,startedAt:Date.now(),flags:{meetingConclusion:'下次再議'},drafts:{},userName:'小雨',sullyId:'ui-sully'};
        market.saveFishingMarketState(state);
    },{id,nodeId,line});
    await button(id.startsWith('C')?'與凱恩交談':'與艾文交談').click();await page.locator('.srf-dialog').waitFor();
...
try{
    ...
    await page.reload();await button('SAR').click();await button('與凱恩交談').waitFor();
    ...
    const before=await page.evaluate(()=>JSON.parse(window.render_game_to_text()));await close();await button('與凱恩交談').click();await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).scene==='C1-01');assert.equal((await page.evaluate(()=>JSON.parse(window.render_game_to_text()))).node,before.node);
    ...
    await close();await button('與艾文交談').click();await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).node==='start');assert.equal(await page.locator('.srf-stage .sar-dialogue-cast__actor').count(),1,'interrupted event restarts');
    ...
    await node('C1-SPECIAL','member-card');await button('就用這個形象').waitFor();assert(await button('繼續對話').isDisabled());await shot('03-membership');await button('就用這個形象').click();await next();await finish();
    ...
    await page.getByRole('button',{name:'朋友入鏡',exact:true}).click();await page.getByRole('button',{name:/Sully/}).last().click();
    await button('拍好了').click();await shot('05-photo-saved');await next();await finish();
    const photo=await page.evaluate(()=>JSON.parse(localStorage.getItem('vr_fishing_market_v1')).sarFamiliarity.souvenirs.find(s=>s.id==='caian-photo'));assert.equal(photo.draft.photo.actors.length,4);assert(photo.draft.caption.includes('朋友們也都在'));
    ...
    await node('A3-E03','button');await button('按下神秘按鈕').click();await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).node==='confetti');await shot('19-mystery-confetti');
    await close();await button('打開倉庫').click();await page.getByTestId('sar-wallet-balance').waitFor();await button('紀念').click();await shot('11-keepsakes');
    await page.getByRole('button',{name:/^第一次 SAR [会會][议議] ·/}).click();await button('收好紀念物').waitFor();await shot('12-keepsake-photo');await button('看看背面').click();await shot('13-photo-back');await page.keyboard.press('Escape');assert.equal(await button('收好紀念物').count(),0);
    ...
    await button('打開收集圖鑑').click();await button('名冊').click();await shot('14-roster-profile');await page.getByRole('button',{name:/^回[忆憶]/}).click();await page.getByText('一星篇章',{exact:true}).click();await button('回顧彼方也太方便了吧').click();
    const saved=await page.evaluate(()=>localStorage.getItem('vr_fishing_market_v1'));assert.equal(await page.locator('.srf-stage .sar-dialogue-cast__actor').count(),1,'replay uses the same solo rule');await shot('15-replay');await next();await button('確實').click();await next();assert.equal(await page.evaluate(()=>localStorage.getItem('vr_fishing_market_v1')),saved);await close();
