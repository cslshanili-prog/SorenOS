...
const enter=async()=>{await button('SAR').click();await page.waitForFunction(()=>{const img=document.querySelector('img[alt="SAR 活動室"]');return img?.complete&&img?.naturalWidth>0&&document.querySelector('.sar-room-canvas')?.clientWidth>0;});};
try{
    await page.goto('http://127.0.0.1:5177/test/fixtures/kanata.html?npcs=show');
    await button('下一頁房間').waitFor();
    await page.evaluate(async()=>{
        ...
        for(const [id,name] of [['hub-aran','阿嵐'],['hub-nora','諾拉']]) await DB.saveCharacter({id,name,avatar:'',systemPrompt:'本地界面測試',vrState:{enabled:true,activityMode:'manual',intervalMinutes:120,currentRoom:'sar'}});
        const fish=(id,ownerId,speciesId)=>({id,ownerId,ownerName:ownerId==='user'?'我':'阿嵐',speciesId,caughtAt:Date.now(),weather:'clear',weatherLabel:'晴朗',weatherSource:'simulated',sizeCm:23.4,quality:2});
    ...
    assert.equal(await button('與凱恩交談').count(),1);assert.equal(await button('與艾文交談').count(),1);
    const tools=await page.locator('.sar-hub-tools').boundingBox();assert(tools.x>150&&tools.width>=135&&tools.x+tools.width<=390);
    await shot('01-room');
    await button('活動室設置').click();await page.getByRole('dialog',{name:'活動室設置'}).waitFor();
    await shot('02-settings');const toggle=page.getByRole('switch',{name:'顯示常駐 NPC'});
    await toggle.click();assert.equal(await toggle.getAttribute('aria-checked'),'false');await button('返回活動室').click();
    assert.equal(await button('與凱恩交談').count(),0);assert.equal(await button('與艾文交談').count(),0);assert.equal(await button('進入水域').count(),1);
    await page.reload();await enter();assert.equal(await button('與凱恩交談').count(),0);
    await button('活動室設置').click();await page.getByRole('switch',{name:'顯示常駐 NPC'}).click();await button('返回活動室').click();assert.equal(await button('與凱恩交談').count(),1);
    await button('打開倉庫').click();await page.getByTestId('sar-wallet-balance').waitFor();
    ...
    await page.keyboard.press('Escape');assert.equal(await page.getByRole('dialog',{name:'隨身倉庫'}).count(),1);assert.equal(await page.locator('.sar-hub-item-detail').count(),0);
    await page.getByRole('combobox',{name:'倉庫主人'}).selectOption('hub-aran');assert.equal(await page.getByTestId('sar-wallet-balance').innerText(),'76');assert.equal(await page.locator('.sar-hub-item').count(),2);
    assert((await page.locator('.sar-hub-items').innerText()).includes('雨鼓魚'));assert(!(await page.locator('.sar-hub-items').innerText()).includes('雲紋鯉'));await shot('05-character-warehouse');
    await page.getByRole('combobox',{name:'倉庫主人'}).selectOption('hub-nora');assert.equal(await page.getByTestId('sar-wallet-balance').innerText(),'0');await shot('06-empty');
    ...
    await page.getByRole('combobox',{name:'倉庫主人'}).selectOption('user');await button('模塊').click();assert.equal(await page.locator('.sar-hub-item').count(),2);
    await page.setViewportSize({width:320,height:680});await button('全部').click();await shot('07-small-warehouse');
    await page.keyboard.press('Escape');assert.equal(await page.getByRole('dialog',{name:'隨身倉庫'}).count(),0);assert.equal(await button('打開倉庫').evaluate(el=>el===document.activeElement),true);await shot('08-small-room');
    for(const name of ['進入水域','進入佈告板','活動室設置','打開倉庫']) {const box=await button(name).boundingBox();assert(box.y>=0&&box.y+box.height<=680,`${name} outside viewport`);}
    await page.setViewportSize({width:1100,height:850});await shot('09-desktop-room');await button('打開倉庫').click();await page.getByTestId('sar-wallet-balance').waitFor();await shot('10-desktop-warehouse');
    // Focus must stay in the panel; closing restores the entrance.
    await button('返回活動室').focus();await page.keyboard.press('Shift+Tab');assert(await page.evaluate(()=>document.querySelector('.sar-hub-panel').contains(document.activeElement)));
    await page.keyboard.press('Escape');await button('返回彼方').click();assert.equal(await page.locator('.vr-sar-light').count(),0);
    await button('下一頁房間').click();await page.getByRole('heading',{name:'往期活動',exact:true}).waitFor();await button('返回世界房間').click();
    ...
    const visit=async(balance,permission=false)=>page.evaluate(async({balance,permission})=>{
        ...
        const char={id:'hub-shop',name:'逛店的阿嵐',avatar:'',systemPrompt:'中文回覆，自己決定。',contextLimit:20,memoryPalaceEnabled:false,vrState:{enabled:true,activityMode:'manual',intervalMinutes:120,currentRoom:'sar'}};
    ...
    modelReply='<ACTIVITY>他在展示台前停了一會兒。</ACTIVITY><NOTE>今天先看看。</NOTE><BUY>NO</BUY><USE_ON_USER>NO</USE_ON_USER>';
    ...
    modelReply='<ACTIVITY>他挑中一枚模塊。</ACTIVITY><NOTE>放在我的倉庫裡，改天再試。</NOTE><BUY>YES</BUY><USE_ON_USER>NO</USE_ON_USER>';
    ...
    modelReply='<ACTIVITY>他把自己的模塊裝在你身上。</ACTIVITY><NOTE>正好你也在，我想試試。</NOTE><BUY>YES</BUY><USE_ON_USER>YES</USE_ON_USER>';
    ...
    trip=await visit(30,true);assert.equal(trip.result.ok,true,JSON.stringify(trip.result));assert.equal(trip.state.accounts['hub-shop'],30);assert(!trip.user.vrState.sarModule);assert(trip.result.activity.includes('沒有購買'));
