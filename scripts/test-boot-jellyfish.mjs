...
try {
 ...
 await page.evaluate(()=>window.dispatchEvent(new Event('boot-ready')));await page.getByText('已進入桌面').waitFor();
 await page.reload();assert.equal(await page.locator('.sully-boot').getAttribute('data-cinematic'),'false');
 await page.getByRole('button',{name:'Soren，輕觸進入'}).focus();await page.keyboard.press('Enter');await page.getByText('已進入桌面').waitFor();
 ...
 await page.goto(url+'?ready');await page.getByText('已進入桌面').waitFor();
 await page.goto('http://127.0.0.1:5183/test/fixtures/boot-choice.html');
 const classic=page.getByRole('button',{name:'原版 壁紙柔光 · 文字浮現'});
 ...
 await page.getByRole('button',{name:'預覽所選開場'}).click();
 await page.locator('[aria-label="Soren"]').waitFor();
 assert.equal(await page.locator('.sully-boot').count(),0);
 await page.locator('[aria-label="Soren"]').click();await jelly.click();await page.getByText('已保存',{exact:true}).waitFor();await page.reload();await jelly.waitFor();
 assert.equal(await jelly.getAttribute('aria-pressed'),'true');
 await page.getByRole('button',{name:'預覽所選開場'}).click();await page.locator('.sully-boot').waitFor();
