...
const getOutput = () => page.getByLabel('導出結果').textContent();
const open = async () => { await page.getByRole('button', { name: '製作分享卡', exact: true }).click(); await page.getByRole('dialog', { name: '製作分享圖片' }).waitFor(); };
const mobileSettings = async (tab = '文字與署名') => {
    const toggle = page.getByRole('button', { name: '調整樣式', exact: true });
...
try {
    ...
    for (const [label, style] of [['留白相紙', 'paper'], ['全幅海報', 'poster'], ['橫版名片', 'business']]) {
        await open();
        await page.getByRole('button', { name: label, exact: true }).click();
        await page.getByLabel('作者名', { exact: true }).fill('Sully 創作室');
        await page.getByLabel('使用限制', { exact: true }).fill('僅限自用 · 禁止商用 · 轉載請署名');
        await page.getByLabel('上傳分享預覽圖').setInputFiles('public/room-templates/forest-cottage/preview.png');
        await page.getByRole('button', { name: '更換預覽圖', exact: true }).waitFor();
        ...
        const saved = await download('導出 PNG 分享圖', `${out}/${style}.png`);
        assert.equal(saved.suggestedFilename(), '月光來信.sully.png');
        ...
        assert.equal(check.metadata.style, style); assert.equal(check.metadata.author, 'Sully 創作室');
        assert.equal(check.pixels, previewPixels);
        assert.equal(check.data.systemPrompt, '你是月光來信。中文 🌙');
    ...
    assert.equal(await page.getByRole('region', { name: '分享設置', exact: true }).isVisible(), false);
    ...
    await mobileSettings('圖片與排版');
    await page.getByLabel('上傳分享預覽圖').setInputFiles('public/room-templates/forest-cottage/preview.png');
    await page.getByRole('button', { name: '更換預覽圖', exact: true }).waitFor();
    await assertSimultaneousPreview();
    await page.screenshot({ animations: 'disabled', path: `${out}/mobile-320-layout.png` });
    for (const name of ['橫版名片', '全幅海報', '留白相紙']) {
        await page.getByRole('button', { name, exact: true }).click();
        await assertSimultaneousPreview();
    }
    await mobileSettings();
    await page.getByLabel('作品名稱', { exact: true }).fill('很長的中文作品名'.repeat(7).slice(0, 60));
    await page.getByLabel('作者名', { exact: true }).fill('作者'.repeat(16));
    await page.getByLabel('使用限制', { exact: true }).fill('僅限自用，轉載請註明作者，不得用於商業用途。'.repeat(6).slice(0, 120));
    ...
    await page.getByRole('button', { name: '收起設置', exact: true }).click();
    assert.equal(await page.getByRole('region', { name: '分享設置', exact: true }).isVisible(), false);
    ...
    await download('導出 PNG 分享圖', `${out}/long-copy.png`);
    ...
    await open(); const original = await download('導出原格式', `${out}/original.json`);
    ...
    await open(); const before = downloads; await page.getByRole('button', { name: '導出 PNG 分享圖' }).click();
    await page.waitForFunction(() => document.querySelector('.sully-share-primary')?.textContent === '導出 PNG 分享圖');
    assert.equal(downloads, before); assert.equal(await page.getByRole('dialog').count(), 1);
    await page.getByRole('button', { name: '關閉分享編輯器' }).click();
    ...
    const cssSection = page.getByRole('region', { name: '真實白框編輯器' });
    await cssSection.getByRole('button', { name: '導出分享', exact: true }).click();
    await download('導出 PNG 分享圖', `${out}/css.png`);
    const cssInput = cssSection.locator('input[accept*=".css"]');
    await cssInput.setInputFiles(`${out}/css.png`);
    assert.equal(await page.getByLabel('CSS 內容').textContent(), '.chat-chrome { color: #887799; } /* 中文 🌙 */');
    const dialogMessage = page.waitForEvent('dialog'); await cssInput.setInputFiles(`${out}/paper.png`);
    const alert = await dialogMessage; assert.match(alert.message(), /[这這]是一[张張]角色卡分享[图圖]/); await alert.accept();
    assert.equal(await page.getByLabel('CSS 內容').textContent(), '.chat-chrome { color: #887799; } /* 中文 🌙 */');
    ...
    await page.waitForFunction(async () => {
        ...
        const chars = await DB.getAllCharacters(); return chars.some(c => c.name === '月光來信' && c.systemPrompt === '你是月光來信。中文 🌙');
    });
