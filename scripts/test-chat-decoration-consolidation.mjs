...
try{
 ...
 const notice=page.getByRole('dialog',{name:'喜歡的樣子，在一處調好。'});await notice.waitFor();
 ...
 await page.reload();await page.getByRole('heading',{name:'外觀定製'}).waitFor();assert.equal(await notice.count(),0);
 ...
 await page.evaluate(async()=>{await window.sarQA.DB.saveCharacter({id:'entry-fix',name:'入口測試',avatar:'',systemPrompt:'測試'});localStorage.setItem('os_last_active_char_id','entry-fix');});
 await page.reload();await notice.waitFor();
 assert.equal(await page.getByRole('complementary',{name:'ChatApp 裝扮'}).count(),0);
 ...
 for(const width of [320,390]){
  ...
  for(let i=1;i<=3;i++){
   await page.getByRole('button',{name:'第 '+i+' 頁',exact:true}).click();
   const grid=page.getByRole('group',{name:'聊天功能第 '+i+' 頁',exact:true});
   ...
   if(i===2){await grid.getByRole('button',{name:'聊天裝扮',exact:true}).waitFor();await grid.getByRole('button',{name:'相冊',exact:true}).waitFor();}
   if(i===3){await grid.getByRole('button',{name:'記憶鏈接',exact:true}).waitFor();await grid.getByRole('button',{name:'收藏',exact:true}).waitFor();}
 ...
 await swipe(-100);assert.equal(await page.getByRole('button',{name:'第 3 頁',exact:true}).getAttribute('aria-current'),'page');
 await swipe(100);assert.equal(await page.getByRole('button',{name:'第 2 頁',exact:true}).getAttribute('aria-current'),'page');
 // A synthesized click after swiping is swallowed; a subsequent deliberate click opens the panel.
 await page.getByRole('button',{name:'聊天裝扮',exact:true}).click();await page.getByRole('button',{name:'聊天裝扮',exact:true}).click();
 await page.getByRole('complementary',{name:'ChatApp 裝扮'}).waitFor();assert.equal(await notice.count(),0);
 ...
 await page.reload();await page.getByRole('complementary',{name:'ChatApp 裝扮'}).waitFor();assert.equal(await notice.count(),0);
 const panel=page.getByRole('complementary',{name:'ChatApp 裝扮'});const state=()=>page.evaluate(()=>window.decorationQA);
 const group=title=>panel.locator('details').filter({has:page.locator('summary').filter({hasText:title})});
 await group('頭部與在線狀態').locator('summary').click();await group('頭部與在線狀態').getByRole('button',{name:'圓點在線',exact:true}).click();
 ...
 await group('氣泡與頭像').locator('summary').click();await group('氣泡與頭像').getByRole('button',{name:'每條都顯示 每條消息都帶頭像',exact:true}).click();
 assert.equal((await state()).char.chatAppearance.chatAvatarMode,'every_message');
 await group('表情包與輸入欄').locator('summary').click();await group('表情包與輸入欄').getByRole('button',{name:'大 160px · 舊版',exact:true}).click();assert.equal((await state()).char.chatAppearance.chatEmojiSize,'large');
 await panel.getByRole('checkbox',{name:/[单單][独獨][调調]整[布佈]局/}).uncheck();assert.equal((await state()).char.chatAppearance.chatEmojiSize,'large');assert.equal((await state()).char.chatFineTune.enabled,false);
 await panel.getByRole('group',{name:'正在設置'}).getByRole('button',{name:'全局默認',exact:true}).click();await group('頭部與在線狀態').locator('summary').click();await group('頭部與在線狀態').getByRole('button',{name:'狀態膠囊',exact:true}).click();assert.equal((await state()).theme.chatStatusStyle,'pill');assert.equal((await state()).char.chatAppearance.chatStatusStyle,'dot');
