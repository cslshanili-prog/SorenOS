...
const backToIdle=async()=>{await btn('打開扭蛋').click();await btn('再看看機器').click();};
try{
    await page.goto(base+'?open=gacha');await page.waitForFunction(()=>!document.querySelector('.sarg-draw-button')?.disabled);
    assert(!(await page.locator('body').innerText()).includes('無限抽取'));
    await shot('01-free');
    await page.locator('.sarg-draw-button').evaluate(el=>{el.click();el.click();});
    await btn('打開扭蛋').waitFor();assert.equal((await read()).ledger.length,1);assert.equal((await read()).accounts.user,200);
    await backToIdle();await shot('02-paid');
    assert((await page.locator('.sarg-draw-button').innerText()).includes('90 鱗幣'));
    await page.locator('.sarg-draw-button').click();await btn('打開扭蛋').waitFor();assert.equal((await read()).accounts.user,110);
    // Closing before reveal must not discard an already granted item.
    await btn('離開扭蛋機').click();await page.reload();await btn('打開扭蛋').waitFor({state:'hidden'});
    assert.equal(Object.values((await read()).sarCommerce.gacha.collection).reduce((a,b)=>a+b,0),2);
    await btn('離開扭蛋機').click();await btn('打開商店').click();await page.waitForFunction(()=>document.querySelector('.sar-module-shop__currency')?.textContent.includes('110'));
    await shot('03-shop');
    ...
    assert((await page.locator('.sar-module-buy').innerText()).includes('鱗幣'));
    ...
    await page.reload();await btn('離開扭蛋機').click();await btn('打開商店').click();
    ...
    await page.getByRole('tab',{name:/模[块塊]袋/}).click();await page.locator('.sar-module-card').first().click();
    await page.locator('.sar-module-buy').click();await btn('繼續').click();
    const installGeometry=[];
    for(const viewport of [{width:390,height:844},{width:320,height:568},{width:740,height:390}]){
        await page.setViewportSize(viewport);await btn('確認裝載').scrollIntoViewIfNeeded();
        ...
        const box=await btn('確認裝載').boundingBox();
    ...
    await btn('確認裝載').click();await page.getByRole('heading',{name:'裝載完成',exact:true}).waitFor();
    ...
    await btn('返回模塊袋').click();await page.getByRole('tab',{name:/今日[货貨]架/}).click();
    ...
    await page.waitForFunction(()=>document.querySelector('.sar-module-shop__currency')?.textContent==='0鱗幣');
    ...
    await btn('關閉模塊詳情').click();await btn('離開模塊商店').click();await btn('打開抽卡').click();
    ...
    await page.locator('.sarg-draw-button').click();await btn('打開扭蛋').waitFor();assert.equal((await read()).accounts.user,0);
    await backToIdle();
    ...
    await page.evaluate(()=>{window.qaSetItem=Storage.prototype.setItem;Storage.prototype.setItem=function(key,value){if(key==='vr_fishing_market_v1')throw new DOMException('無法保存','QuotaExceededError');return window.qaSetItem.call(this,key,value);};});
    ...
    await page.locator('.sarg-draw-button').click();await btn('打開扭蛋').waitFor();assert.equal((await read()).accounts.user,10);
