...
try {
    ...
    await page.evaluate(async () => {
        ...
        await market.mutateFishingMarket(state => {
            for (const [id, speciesId, ownerId] of [['qa-old-fish', 'glass-minnow', 'user'], ['qa-old-dino', 'brachiosaurus', 'user'], ['qa-char-fish', 'glass-minnow', 'qa-facility-0']]) {
                state = market.addCatchToState(state, { id, speciesId, ownerId, ownerName: ownerId === 'user' ? 'user' : '舊名字', caughtAt: Date.now(), weather: 'clear', weatherLabel: '晴天', weatherSource: 'simulated', sizeCm: 12, quality: 2, origin: { kind: 'fished', actorId: ownerId, actorName: ownerId === 'user' ? 'user' : '舊名字', at: Date.now() } });
            }
            return state;
        });
    });
    await button('圖鑑').click();
    await page.getByRole('heading', { name: '雨眠 的水域圖鑑' }).waitFor();
    await page.getByRole('button', { name: /玻璃米[鱼魚]/ }).click();
    const fish = page.getByRole('dialog', { name: '玻璃米魚', exact: true });
    ...
    await button('關閉詳情').click();
    await page.getByLabel('查看誰的錢包和收藏', { exact: true }).selectOption('qa-facility-0');
    await page.getByRole('button', { name: /玻璃米[鱼魚]/ }).click();
    await fish.getByText(/^Sully 的收藏/).waitFor();
    await button('關閉詳情').click();
    await enter('warehouse');
    await button('打開收集圖鑑').click();
    await page.getByRole('button', { name: /^[鱼魚][类類][图圖][鉴鑑] ·/ }).click();
    assert.equal(await page.getByLabel('圖鑑主人', { exact: true }).locator('option[value="user"]').innerText(), '小雨');
    await shot('fish-catalog');
    await rename('雨眠');
    await page.waitForFunction(() => document.querySelector('select[aria-label="圖鑑主人"] option[value="user"]').textContent === '雨眠');
    await enter('garden');
    await button('恐龍').click();
    await page.locator('.clay-collection button').first().click();
    await button('名字與管理').click();
    await button('近看與來歷').click();
    const dino = page.getByRole('dialog', { name: '恐龍圖鑑詳情', exact: true });
    await dino.getByText('雨眠', { exact: true }).waitFor();
    await dino.getByText(/雨眠[钓釣]到了它/).waitFor();
