...
await page.route('**/sar-qa-api/chat/completions',route=>{
    ...
    return route.fulfill(fail?{status:500,json:{error:{message:'API臨時故障'}}}:{json:{choices:[{message:{content:JSON.stringify({worldNarration:'新的旁白',character:'重新生成的角色回覆'})}}]}});
});
...
try{
    await open();
    await btn('第 1 幕回覆操作').click();await btn('複製').click();
    assert((await page.evaluate(()=>navigator.clipboard.readText())).includes('怎麼涼成這樣'));
    await btn('第 1 幕回覆操作').click();await btn('修改').click();
    await page.getByLabel('修改角色回覆',{exact:true}).fill('我修改後的回覆');
    await page.getByLabel('修改世界旁白',{exact:true}).fill('我修改後的旁白');
    await page.screenshot({path:out+'/edit-dialog.png'});
    await btn('保存修改').click();await page.getByText('修改已保存',{exact:true}).waitFor();
    await open();assert(await page.getByText('我修改後的回覆',{exact:true}).isVisible());
    fail=true;await btn('第 1 幕回覆操作').click();await btn('重新生成').click();
    await page.getByRole('alert').waitFor();assert(await page.getByText('我修改後的回覆',{exact:true}).isVisible());
    fail=false;await btn('第 1 幕回覆操作').click();await btn('刪除').click();await btn('確認刪除回覆').click();
    await btn('生成').waitFor();await open();await btn('生成').click();
    await page.getByText('重新生成的角色回覆',{exact:true}).waitFor();
    assert(calls.at(-1).messages.at(-1).content.includes('我坐到他身邊'));
    assert(!JSON.stringify(calls.at(-1).messages).includes('先不管別的了'));
    ...
    await page.setViewportSize({width:320,height:740});await btn('第 2 幕回覆操作').click();
    ...
    await page.getByText('完整原文測試',{exact:true}).click();
    await page.getByRole('button',{name:/已[归歸][档檔] 2 [条條][剧劇]情/}).click();
    await page.getByRole('button',{name:/第 1 段 · 完整原文/}).click();
    const first=page.locator('[data-game-archived-log="l1"]');
    assert((await first.innerText()).includes('第二段結尾標記：舊塔的燈終於亮了。'));
    await page.getByRole('button',{name:/第 2 段 · 完整原文/}).click();
    assert((await page.locator('[data-game-archived-log="l2"]').innerText()).includes('舊版歸檔也保留的尾句'));
