...
await context.route('**/*', async route => {
    ...
    if (url.pathname.includes('/chat/completions')) {
        ...
        const actions=[{actorId:ids[0],action:'post',ref:'n1',title:'魚價辯論第'+modelCalls.length+'回',words:'隔壁說魚會自己砍價，到底誰教的？'},...ids.slice(1).map(actorId=>({actorId,action:'comment',targetId:'n1',words:'你先讓魚開口，我負責記帳。'})),{actorId:ids[0],action:'comment',targetId:'n1',words:'它剛吐的那個泡算不算口頭報價？'}];
...
const refresh = () => page.getByRole('button', { name: '刷新佈告板', exact: true });
...
try {
    ...
    await refresh().click(); await page.getByRole('status').filter({ hasText: '來過了' }).waitFor();
    assert.equal(await page.evaluate(() => window.facilityTripCalls?.length || 0), 0, 'the NPC roll does not invoke a character');
    assert.equal(modelCalls.length,1);
    await refresh().click(); await page.waitForFunction(()=>!document.querySelector('[aria-label="刷新佈告板"]').disabled);assert.equal(modelCalls.length,2);
    const board=await page.evaluate(()=>JSON.parse(localStorage.getItem('vr_fishing_market_v1')));assert.equal(board.requests.filter(p=>p.itemLabel.startsWith('魚價辯論')).length,2);assert.equal(board.requests.filter(p=>p.itemLabel.startsWith('魚價辯論'))[0].comments.length,2);
    ...
    await page.getByRole('status').filter({ hasText: 'Sully來過了' }).waitFor();
    await page.getByRole('button', { name: '指定角色', exact: true }).click();
    await page.getByLabel('選擇逛佈告板的角色', { exact: true }).selectOption('qa-facility-1');
    const invite = page.getByRole('button', { name: '讓 ta 逛佈告板', exact: true });
    ...
    await page.getByRole('button', { name: '返回佈告板', exact: true }).click();
    ...
    await refresh().click();await page.waitForFunction(()=>!document.querySelector('[aria-label="刷新佈告板"]').disabled);assert.equal(modelCalls.length,3);
    ...
    const count=await page.evaluate(()=>JSON.parse(localStorage.getItem('vr_fishing_market_v1')).requests.length);failNext=true;await refresh().click();await page.waitForFunction(()=>!document.querySelector('[aria-label="刷新佈告板"]').disabled);assert.equal(modelCalls.length,4);assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('vr_fishing_market_v1')).requests.length),count);
    await page.evaluate(()=>{const key='vr_sar_club_state_v1';localStorage.setItem(key,JSON.stringify({...JSON.parse(localStorage.getItem(key)||'{}'),npcPreference:'hide'}));});await refresh().click();await page.waitForFunction(()=>!document.querySelector('[aria-label="刷新佈告板"]').disabled);assert.equal(modelCalls.length,4);
