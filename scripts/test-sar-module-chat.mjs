...
try{
    ...
    assert((await page.locator('.qa-message').nth(10).innerText()).includes('把這詛咒之物給我剝離！'));await shot('surface-390');
    const toggles=page.getByRole('button',{name:'查看原台詞',exact:true});assert.equal(await toggles.count(),9);
    while(await toggles.count())await toggles.first().click();
    assert((await page.locator('.qa-message').nth(9).innerText()).includes('現在，立刻，馬上。'));assert((await page.locator('.qa-message').nth(10).innerText()).includes('把這破插件給我卸了！'));
    ...
    assert((await page.locator('.qa-message').nth(10).innerText()).includes('把這詛咒之物給我剝離！'));assert.deepEqual(await read(),before,'reload retains the same paired messages');
