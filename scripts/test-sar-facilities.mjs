...
const guide = async (facility, name) => {
    ...
    await btn('關閉玩法引導').click(); assert.equal(await panel.count(), 0);
};
...
try {
    ...
    await btn('史冊下一頁').click(); await btn('史冊下一頁').click(); assert.equal(await page.locator('.sarc-library-book').count(), 5);
    await btn('選擇角色 角色 01').click(); assert((await page.locator('.sarc-library-owner').innerText()).includes('角色 01'));
    assert.equal(await page.locator('.sarc-library-book').count(), 0, 'empty characters remain selectable');
    await page.getByLabel('角色分組', { exact: true }).selectOption('qa-b');
    assert.equal(await page.locator('.sar-character-picker-grid button').count(), 8);
    await btn('角色下一頁').click(); await btn('選擇角色 角色 38').click();
    ...
    await btn('選擇角色 角色 59').click(); await page.getByRole('tab', { name: '角色的隨筆', exact: true }).click();
    ...
    await page.getByRole('alert').waitFor(); assert.equal(await page.getByText('還沒留下隨筆', { exact: true }).count(), 0);
    await shot('cabinet-read-retry'); await btn('重新讀取隨筆').click();
    await page.getByText('還沒留下隨筆', { exact: true }).waitFor(); assert.equal(await page.getByRole('alert').count(), 0);
    ...
    await btn('選擇角色 角色 59').waitFor();
    await page.getByLabel('角色分組', { exact: true }).selectOption('qa-b'); await page.getByLabel('搜索角色', { exact: true }).fill('47');
    await btn('選擇角色 角色 47').click(); await shot('forge-grouped-character');
    await enter('warehouse'); await guide('warehouse', '凱恩');
    assert.equal(await page.locator('.sar-hub-items>.sar-hub-item').count(), 12);
    assert.equal(await page.getByLabel('倉庫主人', { exact: true }).locator('option[value="user"]').innerText(), '我');
    const first = await page.locator('.sar-hub-item').first().getAttribute('aria-label');
    await btn('倉庫物品下一頁').click(); assert.notEqual(await page.locator('.sar-hub-item').first().getAttribute('aria-label'), first);
    await shot('warehouse-paged');
    await page.setViewportSize({ width: 320, height: 640 }); await noOverflow(); await shot('warehouse-320');
    await page.getByLabel('倉庫主人', { exact: true }).selectOption('qa-facility-59');
    assert.equal(await page.getByRole('navigation', { name: '倉庫物品分頁' }).count(), 0);
    await page.setViewportSize({ width: 390, height: 844 });
    await enter('gacha'); await guide('gacha', '凱恩');
    await page.getByRole('button', { name: /[两兩]枚芯片，一段[异異]世界 · 玩法[说說]明/ }).click();
    assert((await page.locator('[data-sar-guide]').innerText()).includes('異界座標'));
    ...
    await enter('water'); await guide('water', '艾文'); await btn('簡單').click();
    ...
    await page.evaluate(() => { Math.random = () => .99; }); await btn('再釣一次').click(); await page.evaluate(() => window.advanceTime(4400));
    assert.equal((await state()).phase, 'escaped'); assert.equal((await market()).inventory.length, before + 1); await shot('simple-empty');
    await enter('board'); await guide('board', '凱恩'); await shot('board-help-entry');
    await enter('modules'); await guide('modules', '凱恩'); await shot('modules-help-entry');
