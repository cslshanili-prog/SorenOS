...
try{
 await page.goto(`${base}/test/fixtures/fishing.html`);await button('拋竿').waitFor();await shot('01-manual-idle');
 ...
 await button('簡單').click();await button('拋竿').click();await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).phase==='caught'&&!JSON.parse(window.render_game_to_text()).saving);
 assert.equal((await read()).inventory.length,1);assert.equal((await state()).simple,true);await shot('02-simple-catch');
 await button('再釣一次').click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('vr_fishing_market_v1')).inventory.length===2);await page.reload();await button('拋竿').waitFor();assert.equal(await button('簡單').getAttribute('aria-pressed'),'true');
 await button('手動').click();await button('拋竿').click();await page.evaluate(()=>window.advanceTime(0));
 for(let i=0;i<320;i++){
 ...
 await button('再釣一次').click();await button('收竿').click();assert.equal((await read()).inventory.length,3);
 await button('再釣一次').click();await page.evaluate(()=>window.advanceTime(35000));assert.equal((await state()).phase,'escaped');assert.equal((await read()).inventory.length,3);
 await button('釣魚說明').click();await page.getByRole('dialog',{name:'釣魚說明',exact:true}).waitFor();await shot('04-help');await button('關閉釣魚說明').click();
 await page.getByText('角色釣魚',{exact:true}).click();assert.equal(await page.getByLabel('選擇去水域的角色').isVisible(),true);await shot('05-character-panel');await page.getByText('角色釣魚',{exact:true}).click();
