...
应用统一显示为 `Soren`。根目录 `capacitor.config.json` 的 `appName` 是原生打包名称；同步脚本会调用 `scripts/sync-native-app-name.mjs`，将外部 CHICK2 包装工程的 Capacitor `appName`、Android `app_name` 与 `title_activity_main` 一起更新。包名、appId 和 URL scheme 保持不变。此操作随下次 APK 构建生效，不会靠网页更新改写已安装 APK 的系统标签。
