...
it('reading memorial does no schema, migration, lock, flag or seed writes and retains unfinished poem',async()=>{
    ...
    const db={prepare:vi.fn((sql:string)=>{queries.push(sql);expect(sql.trim()).toMatch(/^SELECT/);
        const stmt={bind:()=>stmt,first:async()=>sql.includes('FROM po_booklets')?{id:'book',title:'原冊',poems_target:20}:sql.includes("status = 'open'")?{id:'unfinished',title:'原詩',status:'open'}:null,
            all:async()=>({results:sql.includes('FROM po_poem_lines')?[{seq:1,device:'mine',pen:'原筆名',content:'原句'}]:[]}),run:vi.fn(()=>{throw Error('write');})};return stmt;}),exec:vi.fn(()=>{throw Error('schema');})};
    const r=await worker.fetch(request('/po/poem/current?device=mine'),{DB:db} as any);expect(r.status).toBe(200);
    expect(await r.json()).toMatchObject({ended:true,paused:true,booklet:{poemsTarget:20},poem:{title:'原詩',lines:[{content:'原句',mine:true}]}});expect(db.exec).not.toHaveBeenCalled();
