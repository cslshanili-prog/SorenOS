...
function Preview(){
 ...
 return <>
  ...
  <div className="sully-chat-root" style={{height:'100dvh',background:background?`url("${background}") center / cover`:'#f7f5fa',color:'#584b64',fontFamily:'system-ui',padding:'40px 24px 105px',overflowY:'auto'}}>
   ...
   <div style={{marginTop:24,maxWidth:260,padding:'14px 18px',background:'white',borderRadius:'18px 18px 18px 4px',fontSize:14}}>換個你喜歡的樣子吧。</div>
   <div style={{margin:'18px 0 0 auto',maxWidth:210,padding:'14px 18px',background:bubble?.user.backgroundColor,color:'white',borderRadius:18,fontSize:14}}>那就從這裡開始。</div>
   ...
   <form className="sully-chat-inputbar" style={{position:'fixed',bottom:0,left:0,right:0,background:'#faf9fc',borderTop:'1px solid #e8e1ef',paddingBottom:'env(safe-area-inset-bottom,0px)'}} onSubmit={event=>{event.preventDefault();if(draft.trim()){setSent(messages=>[...messages,draft.trim()]);setDraft('');}}}>
    <div className="sully-chat-composer" style={{display:'flex',alignItems:'center',gap:10,padding:'14px 16px'}}>
     <button type="button" className="sully-chat-actions-button" aria-label="打開裝扮" onClick={()=>setOpen(true)} style={{border:0,background:'none',fontSize:28,color:'#9684a6',padding:4}}>＋</button>
     <div className="sully-chat-input-wrap" style={{display:'flex',flex:1,minWidth:0,borderRadius:24,background:'#eee9f3',padding:'8px 14px'}}><input aria-label="預覽消息" placeholder="說點什麼…" value={draft} onChange={e=>setDraft(e.target.value)} style={{minWidth:0,width:'100%',border:0,outline:0,background:'transparent',fontSize:14,color:'#584b64',lineHeight:'28px'}}/><span aria-hidden="true" style={{alignSelf:'center',color:'#a594b5'}}>☺</span></div>
     <button type="submit" className="sully-chat-send-button" aria-label="發送預覽消息" style={{border:0,borderRadius:22,background:'#8b739e',color:'white',padding:'12px 14px',fontSize:12}}>發送</button>
    </div>
   </form>
   {!open&&<button onClick={()=>setOpen(true)} style={{marginTop:24}}>打開裝扮</button>}
   {workshop&&<p role="status">氣泡工坊入口已觸發（預覽不離開頁面）</p>}
  </div>
