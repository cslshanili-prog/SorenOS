...
const base={...createFishingMarketState(20260911),inventory:[{id:catchId,speciesId,ownerId:'user',ownerName:'試玩玩家',caughtAt:now,weather:'clear' as const,weatherLabel:'艾文的贈禮',weatherSource:'simulated' as const,sizeCm:12,quality:1 as const,origin:{kind:'gift' as const,actorId:'sar-evan',actorName:'艾文',at:now}}],dinosaurGarden:{version:2 as const,gridVersion:1 as const,activeMapId:'grassland',maps:createGardenMaps(),toys:{},events:[],visitsEnabled:false,revision:0}};
...
function Preview(){
  ...
  return <><header><span className="icon"><DinoIcon species={speciesId} color={paint.body}/></span><div><h1>？？？</h1><p>艾文的特殊恐龍 ·「不知道是什麼。所以不用糾正。」</p></div></header><div className="model" ref={host}/><nav><button onClick={()=>setView(v=>v==='portrait'?'garden':'portrait')}>{view==='portrait'?'放進箱庭':'查看模型'}</button><button onClick={()=>setPaint({body:'#e0bb80',accent:'#b88cad'})}>試試換色</button><button onClick={()=>setPaint(defaultDinoPaint(speciesId))}>原始配色</button><button onClick={()=>engine.current?.greet()}>打招呼</button><button onClick={()=>engine.current?.reset()}>重置視角</button></nav>{error&&<p role="alert">{error}</p>}</>;
}
