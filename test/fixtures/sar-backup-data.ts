...
export async function seedSARBackup(os: any) {
    ...
    const char = { ...os.characters[0], id:'qa-sar-backup', name:'備份測試角色', chatBackground:ANNIVERSARY_WALLPAPERS[1].url, bubbleStyle:theme.id,
        vrState:{ enabled:false, currentRoom:'sar', sarActivity:'garden', title:'測試稱號', sarModule:runtime } };
    await DB.saveCharacter(char);
    await DB.saveTheme(theme);
    await DB.saveUserProfile({ ...os.userProfile, name:'備份測試用戶', vrState:{ enabled:false, currentRoom:'sar', title:'測試稱號', sarModule:runtime } });
    await os.updateTheme({ wallpaper:ANNIVERSARY_WALLPAPERS[0].url });
    await DB.saveMessage({ charId:char.id, role:'assistant', type:'text', content:'真意', timestamp:at, metadata:{ sarModuleSurface:{ moduleId:runtime.moduleId, moduleTitle:runtime.moduleTitle, surface:'表面說法', trueText:'真意' } } } as any);
    ...
    market.sarFamiliarity.npcs.caian = { stars:1, completed:{'C1-01':{at,flags:{answer:'保留'}}}, day:'2026-09-11', offerId:'C1-02', queuedSceneIds:['qa-pending'],
        pending:{runId:'qa-run',sceneId:'C1-02',nodeId:'start',line:1,revision:2,startedAt:at,flags:{},drafts:{qa:{text:'未完成草稿'}},userName:'備份測試用戶'} };
    market.sarFamiliarity.applied = ['qa-reward'];
    market.sarFamiliarity.souvenirs = [{id:'qa-photo',title:'測試照片',description:'測試紀念',npc:'caian',sceneId:'C1-01',nodeId:'finish',at,userName:'我',flags:{},draft:{photo,legacyPhoto:png}}];
    ...
    localStorage.setItem('vr_sar_simulations_v1',JSON.stringify({version:1,records:[{id:'qa-story',story:'已保存推演',draft:'輸入草稿',photo}]}));
