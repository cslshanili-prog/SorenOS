...
const errors=[],consoleErrors=[];let calls=0;let response={disposition:'keep',reaction:'這條魚透著光，很好看。',shareToUser:{text:'你看，我剛釣上來的。'}};let fail=false;
...
const waitDone=async()=>{await page.waitForFunction(()=>!document.querySelector('.fishing-shell')?.textContent.includes('活動進行中'));await page.waitForTimeout(350);};
const choose=async id=>{if(!await page.getByLabel('選擇去水域的角色').isVisible())await page.getByText('角色釣魚',{exact:true}).click();await page.getByLabel('選擇去水域的角色').selectOption(id);};
const trip=async()=>{await page.getByRole('button',{name:'讓 ta 去釣魚',exact:true}).click();await waitDone();};
try{
    ...
    await page.getByRole('button',{name:'查看 ta 的收藏與圖鑑 →'}).click();await page.getByText('阿嵐 的水域圖鑑',{exact:true}).waitFor();await page.screenshot({path:out+'/personal-catalog-390.png'});
    await page.getByLabel('查看誰的錢包和收藏').selectOption('fish-b');assert.equal(await page.getByText('累計獲得 1 件 · 現有 1 件',{exact:true}).count(),0);
    await page.getByRole('button',{name:'釣魚',exact:true}).click();await choose('fish-b');response={disposition:'release',reaction:'還小，放回水裡吧。',shareToUser:{text:'我放回去了。今天算陪它散步。'}};
    ...
    await page.getByRole('button',{name:'繼續處理這一竿',exact:true}).waitFor();await page.screenshot({path:out+'/pending-390.png'});
    fail=false;response={disposition:'keep',reaction:'留下了。',shareToUser:null};await page.getByRole('button',{name:'繼續處理這一竿',exact:true}).click();await waitDone();s=await state();assert.equal(s.fishingTrips.length,count);assert.equal(s.fishingTrips.find(t=>t.catch.id===pendingId).status,'settled');
    ...
    response={disposition:'keep',reaction:'想給你看看。',shareToUser:{text:'這是剛才那一條。'}};await trip();const beforeRetry=calls;await page.getByRole('button',{name:'重試發送記錄與分享'}).waitFor();
    await page.evaluate(()=>{window.fishingFixture.DB.saveMessageOnce=window.fishingFixture.originalSave;});await page.getByRole('button',{name:'重試發送記錄與分享'}).click();await page.waitForFunction(()=>window.fishingFixture.readFishingMarketState().fishingTrips.at(-1).shareSent);assert.equal(calls,beforeRetry);
    await page.getByRole('button',{name:'查看 ta 的收藏與圖鑑 →'}).click();await page.setViewportSize({width:320,height:740});await page.screenshot({path:out+'/personal-catalog-320.png'});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth),320);
    await page.getByText('累計獲得 3 件 · 現有 3 件',{exact:true}).scrollIntoViewIfNeeded();await page.waitForTimeout(350);await page.screenshot({path:out+'/catalog-stats-320.png'});
    ...
    await page.getByText('留言簿',{exact:true}).click({timeout:60000});await page.getByText('留言牆',{exact:true}).waitFor();await page.getByText('圖鑑解鎖',{exact:true}).first().waitFor();await page.screenshot({path:out+'/public-announcements-320.png'});
