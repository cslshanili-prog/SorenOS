    ...
    return <div className='absolute inset-0 z-40 bg-slate-950/35 flex items-end sm:items-center justify-center' onClick={onClose}>
        <section onClick={event => event.stopPropagation()} className='w-full max-w-xl max-h-[82%] rounded-t-[28px] sm:rounded-[28px] bg-stone-100 shadow-2xl flex flex-col overflow-hidden'>
            <header className='shrink-0 px-5 pt-5 pb-4 border-b border-slate-200'>
                ...
                <div className='mt-3 flex items-center justify-between text-[9px] text-slate-400'><span>僅覆蓋本劇情，不修改預設庫</span><span>當前預設約 {tokenEstimate.toLocaleString()} tokens</span></div>
            </header>
            ...
            <main className='flex-1 overflow-y-auto px-5 py-6'>
                ...
                </> : <div className='py-12 text-center text-xs text-slate-400'>這個預設沒有可用於默認版的選項</div>}
            </main>
            ...
            <footer className='story-safe-sheet shrink-0 px-5 pt-3 border-t border-slate-200'>
                ...
                <div className='mt-3 flex gap-2'>{hasOverride && <button onClick={async () => { await onReset(); onClose(); }} className='px-4 py-3 rounded-xl border border-slate-200 text-[10px] font-bold text-slate-500'>恢復原預設</button>}<button disabled={saving || !current} onClick={apply} className='flex-1 py-3 rounded-xl bg-slate-900 text-white text-xs font-bold disabled:opacity-40'>{saving ? '應用中' : '應用到本劇情'}</button></div>
            </footer>
        </section>
    </div>;
};
