...
interface MemoryArchivistProps {
    ...
    /**
     * 按日期強制從原始聊天重總結（忽略 hideBefore）。日期格式 YYYY-MM-DD。
     * overridePromptId 可選——用戶在重總結小彈窗裡選的模板 id。不提供則走調用方默認。
     */
    onForceArchiveDate?: (dateStr: string, overridePromptId?: string) => Promise<void>;
    /** 可選：傳入歸檔模板列表 + 默認選中 id，用於重總結前讓用戶選模板（避開和內部月度精煉模板 state 同名） */
...
const MemoryArchivist: React.FC<MemoryArchivistProps> = ({ memories, refinedMemories, activeMemoryMonths, charName, userName, onRefine, onDeleteMemories, onUpdateMemory, linkedMemoryEnabled, onToggleActiveMonth, onUpdateRefinedMemory, onDeleteRefinedMemory, onForceArchiveDate, forceArchiveTemplates, forceArchiveDefaultPromptId }) => {
    ...
    // 重總結前彈出模板選擇器：把 date 存起來打開 modal
    ...
    const saveEditedMemory = async () => {
        ...
        catch (error) { setMemorySaveError(error instanceof Error ? error.message : '保存失敗，請重試'); }
    ...
    const triggerRefine = async () => {
        ...
        const combinedText = monthMems.map(m => `${m.date}: ${m.summary} (${m.mood || '無'})`).join('\n');
        ...
        if (templateObj) {
            ...
            // ${rawLog} 不再當場替換成 combinedText：rawText 由 handleRefineMonth 以
            // role:user 單獨投餵（Gemini 3.1 preview 對"規則/身份 → 遲到 rawLog"
            // 的 all-in-one user 消息會靜默拒答，拆開再發能解決）。這裡只留佔位提示。
            formattedPrompt = templateObj.content
                ...
                .replace(/\$\{rawLog.*?\}/g, '<見 user 消息裡的本月日記原件>');
            formattedPrompt = `[角色記憶精煉: ${charName} - ${dateStr}]\n${formattedPrompt}`;
        }
    ...
    if (Object.keys(tree).length === 0) return <div className="flex flex-col items-center justify-center h-48 text-slate-400"><p className="text-xs">暫無記憶檔案</p></div>;
        ...
        <div className="grid grid-cols-2 gap-3 animate-fade-in">
            {Object.keys(tree).map(year => (
                <div key={year} onClick={() => handleYearClick(year)} className="bg-white/60 backdrop-blur-sm p-4 rounded-2xl border border-white/50 shadow-sm active:scale-95 transition-all flex flex-col justify-between h-28 group cursor-pointer hover:bg-white/80">
                    ...
                    <div><h3 className="text-xl font-light text-slate-800 tracking-tight">{year}</h3><p className="text-[10px] text-slate-400">年度檔案歸檔</p></div>
                </div>
            ))}
        </div>
    );
                         ...
                         <div onClick={() => handleMonthClick(month)} className="bg-white/50 backdrop-blur-sm p-3 rounded-2xl border border-white/40 shadow-sm active:scale-95 transition-all flex flex-col justify-center items-center gap-2 aspect-square cursor-pointer hover:bg-white/70 relative overflow-hidden">
                            ...
                            <span className="text-[10px] text-slate-400">
                                {tree[viewState.selectedYear!][month].length > 0
                                    ? `${tree[viewState.selectedYear!][month].length} 條日度`
                                    : (refinedMemories?.[monthKey] ? '僅月度核心' : '無日度記錄')}
                            </span>
                        </div>
        ...
        return (
            <div className="space-y-6 animate-fade-in pb-8">
                <div className="bg-indigo-50/50 rounded-2xl p-4 border border-indigo-100 relative group">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex gap-2">
                             ...
                             <button
                                 ...
                                 title={rawMemories.length === 0 ? '沒有日度記錄，無法重新精煉' : undefined}
                             ...
                             >
                                 {isRefining ? '...' : (refinedContent ? '重新精煉' : '生成')}
                             </button>
                        </div>
                    </div>
                    ...
                    {showPromptPanel && (
                        <div className="mb-3 bg-white/70 p-3 rounded-xl border border-indigo-100 animate-fade-in">
                            <label className="text-[9px] font-bold text-indigo-400 uppercase mb-2 block">選擇總結提示詞</label>
                            ...
                            <p className="text-[8px] text-indigo-300 mt-2 leading-tight">角色月度精煉專用提示詞，與聊天歸檔獨立。</p>
                        </div>
                    )}
                    ...
                    {rawMemories.length === 0 && refinedContent && (
                        <p className="mt-2 text-[10px] leading-relaxed text-indigo-500">
                            本月沒有日度記錄，但這條月度核心記憶仍會發送給角色。長按上方內容可編輯或刪除。
                        </p>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between px-1">
                    ...
                    <div className="flex gap-2">
                        {isManageMode && selectedIds.size > 0 && <button onClick={(e) => { e.stopPropagation(); requestDelete(); }} className="text-[10px] bg-red-500 text-white px-3 py-1 rounded-full font-bold shadow-sm active:scale-95 transition-transform">刪除 ({selectedIds.size})</button>}
                    ...
                    {Object.entries(groupedByDay).map(([date, dayMemories]) => (
                        <div key={date} className="relative pl-8 pb-8 last:pb-0 border-l-[2px] border-slate-100 last:border-l-0 last:border-image-source-none">
                            ...
                            <div className="mb-3 -mt-1.5 flex items-center gap-2">
                                ...
                                {dayMemories.length > 1 && <span className="text-[9px] px-1.5 py-0.5 bg-slate-100 rounded-md text-slate-400 font-normal">{dayMemories.length} 記錄</span>}
                                {onForceArchiveDate && (
                                    <button
                                        onClick={(e) => { e.stopPropagation(); openForcePicker(date); }}
                                        disabled={forcingDate === date}
                                        title={`從原始聊天重新總結 ${date}（忽略已隱藏狀態）`}
                                ...
                                {dayMemories.map((mem) => (
                                    ...
                                    >
                                        ...
                                        <div className={`bg-white p-4 rounded-xl rounded-tl-none border border-slate-100 shadow-sm hover:shadow-md hover:border-primary/20 transition-all relative ${isManageMode && selectedIds.has(mem.id) ? 'ring-2 ring-primary ring-offset-2' : ''}`} onClick={(e) => { if (!isManageMode) { e.stopPropagation(); toggleMemoryExpanded(mem.id); } }}>
                                            ...
                                            {!isManageMode && mem.summary.length > 120 && <div className="mt-2 text-[10px] text-slate-400">{expandedMemoryIds.has(mem.id) ? '點擊收起' : '點擊展開'}</div>}
                                        </div>
                                    </div>
                                ))}
                            </div>
        ...
        <div className="flex flex-col h-full relative">
            <div className="flex justify-between items-center mb-6 px-1">
                <div className="flex gap-4">
                    <div><span className="block text-[10px] text-slate-400 uppercase tracking-widest">總字數</span><span className="text-lg font-medium text-slate-700 font-mono">{stats.totalChars.toLocaleString()}</span></div>
                    <div><span className="block text-[10px] text-slate-400 uppercase tracking-widest">總條目</span><span className="text-lg font-medium text-slate-700 font-mono">{stats.count}</span></div>
                </div>
                <div className="flex items-center gap-1 text-xs font-medium text-slate-500 bg-white/50 px-3 py-1.5 rounded-full border border-white/50 shadow-sm">
                    {viewState.level === 'root' ? <span>檔案室</span> : (
                        <>
                            <button onClick={() => setViewState({level: 'root', selectedYear: null, selectedMonth: null})} className="hover:text-primary">檔案</button><span className="text-slate-300">/</span>
            ...
            {viewState.level === 'year' && <><div className="mb-4 flex items-center gap-2"><button onClick={handleBack} className="p-1.5 bg-white rounded-full text-slate-400 hover:text-slate-600 shadow-sm"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path fillRule="evenodd" d="M17 10a.75.75 0 0 1-.75.75H5.612l4.158 3.96a.75.75 0 1 1-1.04 1.08l-5.5-5.25a.75.75 0 0 1 0-1.08l5.5-5.25a.75.75 0 1 1 1.04 1.08L5.612 9.25H16.25A.75.75 0 0 1 17 10Z" clipRule="evenodd" /></svg></button><h3 className="text-sm font-medium text-slate-600">選擇月份</h3></div>{renderMonths()}</>}
            {viewState.level === 'month' && <><div className="mb-4 flex items-center gap-2"><button onClick={handleBack} className="p-1.5 bg-white rounded-full text-slate-400 hover:text-slate-600 shadow-sm"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path fillRule="evenodd" d="M17 10a.75.75 0 0 1-.75.75H5.612l4.158 3.96a.75.75 0 1 1-1.04 1.08l-5.5-5.25a.75.75 0 0 1 0-1.08l5.5-5.25a.75.75 0 1 1 1.04 1.08L5.612 9.25H16.25A.75.75 0 0 1 17 10Z" clipRule="evenodd" /></svg></button><h3 className="text-sm font-medium text-slate-600">本月記憶 (點擊眼睛圖標激活詳細回憶)</h3></div>{renderMemories()}</>}

            <Modal isOpen={!!editMemory} title="編輯記憶" onClose={() => { if (!savingMemory) setEditMemory(null); }} footer={<button disabled={savingMemory || !editMemory?.summary.trim()} onClick={() => void saveEditedMemory()} className="w-full py-3 bg-primary text-white font-bold rounded-2xl disabled:opacity-50">{savingMemory ? '保存中…' : '保存修改'}</button>}>
                {editMemory && <div className="space-y-3"><div className="text-xs text-slate-400">日期: {editMemory.date}</div>{linkedMemoryEnabled && editMemory.palaceMemoryId && <p className="text-xs text-violet-600">此條關聯記憶宮殿。修改正文會同步修改宮殿並更新向量，可能產生 Embedding API 費用。</p>}<textarea disabled={savingMemory} value={editMemory.summary} onChange={e => setEditMemory({...editMemory, summary: e.target.value})} className="w-full h-40 bg-slate-100 rounded-xl p-3 text-sm resize-none focus:outline-primary"/>{memorySaveError && <p role="alert" className="text-xs text-red-500">{memorySaveError}</p>}</div>}
            </Modal>
            
            <Modal isOpen={showDeleteConfirm} title="確認刪除" onClose={() => setShowDeleteConfirm(false)} footer={<div className="flex gap-2 w-full"><button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-2xl">取消</button><button onClick={performDelete} className="flex-1 py-3 bg-red-500 text-white font-bold rounded-2xl shadow-lg shadow-red-200">確認刪除</button></div>}>
                <p className="text-sm text-slate-600 text-center py-4">確定刪除選中的 {selectedIds.size} 條記憶嗎？<br/><span className="text-xs text-red-400 mt-1 block">此操作不可恢復。</span></p>
                {memories.some(memory => selectedIds.has(memory.id) && memory.palaceMemoryId) && <p className="text-xs text-slate-500">這裡只刪除神經鏈接的檔案記錄，關聯的宮殿記憶仍保留。</p>}
            </Modal>
            ...
            <Modal 
                ...
                footer={
                    <div className="flex gap-2 w-full">
                        <button onClick={() => setShowCoreDeleteConfirm(true)} className="flex-1 py-3 bg-red-50 text-red-500 font-bold rounded-2xl">刪除</button>
            ...
            <Modal isOpen={showCoreDeleteConfirm} title="刪除確認" onClose={() => setShowCoreDeleteConfirm(false)} footer={<div className="flex gap-2 w-full"><button onClick={() => setShowCoreDeleteConfirm(false)} className="flex-1 py-3 bg-slate-100 font-bold rounded-2xl">取消</button><button onClick={confirmCoreDelete} className="flex-1 py-3 bg-red-500 text-white font-bold rounded-2xl">確認刪除</button></div>}>
                <p className="text-center text-sm text-slate-600 py-4">確定要刪除該月的核心記憶嗎？<br/><span className="text-xs text-red-400">刪除後會丟失該月給角色用的核心記憶摘要。</span></p>
            </Modal>
            ...
            <Modal
                ...
                footer={<div className="flex gap-2 w-full">
                    ...
                    <button onClick={confirmForcePicker} className="flex-1 py-3 bg-primary text-white font-bold rounded-2xl">開始總結</button>
                </div>}
            >
                <div className="space-y-3">
                    <p className="text-xs text-slate-500 leading-relaxed">
                        即將從原始聊天重新總結 <b className="text-slate-700">{forcePickerDate}</b> 這一天。
                        此操作忽略隱藏起點，直接讀當天全部原始消息。選擇你想用的提示詞風格：
                    </p>
