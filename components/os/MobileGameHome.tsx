...
// ===== 手遊主題（mobilegame skin）=====
// 風格：夢幻粉紫二次元手遊首頁（照搬參考圖）。淺粉紫底 + 深紫文字 + 粉色強調，
// 圓潤可愛字體、滿屏星芒 ✦、時鐘 Hero 卡內嵌天空漸變 + 角色立繪貼紙。
// 背景透出用戶自設壁紙（默認鋪夢幻粉紫漸變壁紙）。
// 等級 / 經驗 / 貨幣為按角色 id 穩定派生的裝飾數值。

// —— 調色板：全部指向 CSS 變量（根節點按方案生成；默認 = 經典夢幻粉紫）——
const PAL = {
    ink: 'var(--mg-ink)',     // 主文字
    grape: 'var(--mg-grape)', // 標題
    lilac: 'var(--mg-lilac)', // 次文字
    pink: 'var(--mg-pink)',   // 強調淺
    hot: 'var(--mg-hot)',     // 強調深（+ / NEW / 高光）
    peri: 'var(--mg-peri)',   // 鄰色
    mist: 'var(--mg-mist)',   // 極淺底
    cloud: 'var(--mg-cloud)', // 近白底
};

// 方案 → 手遊皮膚整套 CSS 變量。明暗兩套：亮色=現在的夢幻粉彩（透壁紙），
// 暗色=深煙卡+亮字+夜空 Hero（有用戶就好這口）。角色 S/L 定死只轉色相。
const makeMgVars = (st: TgStyle): React.CSSProperties => {
    ...
    return {
        ...
        '--mg-bg': 'transparent', // 亮色保持透出用戶壁紙
    } as React.CSSProperties;
};

// 經典夢幻粉紫（本皮膚的出廠默認，亮色）+ 共享 12 方案
const MG_CLASSIC: TgStyle = { id: 'classic', name: '夢幻粉紫', hue: 262, dark: false, gold: false, mute: false };
...
// 扁平手繪卡（二次元風格）：淺色底 + 輕描邊 + 很淡的平面投影，不要鼓凸
const CARD = {
...
const FONT_DISPLAY = `'DM Serif Display', serif`;     // 大時鐘 / Lv / 日期數字
const FONT_CN = `'ZCOOL KuaiLe', 'Noto Sans SC', sans-serif`; // 中文圓潤可愛
const FONT_SCRIPT = `'Caveat', cursive`;              // 問候手寫

const QUICK_ENTRIES: { id: AppID; cn: string }[] = [
    { id: AppID.Character, cn: '神經鏈接' },
    { id: AppID.MemoryPalace, cn: '記憶宮殿' },
    { id: AppID.Call, cn: '電話' },
    { id: AppID.Room, cn: '小小窩' },
];
...
// 有手遊插畫就用插畫（自帶配色），否則回退到 Phosphor 線性圖標
...
const MobileGameHome: React.FC = () => {
    ...
    // 界面配色方案：localStorage 持久化；默認經典夢幻粉紫（亮）
    ...
    useEffect(() => {
        ...
        DB.getMessagesByCharId(target.id).then(msgs => {
            ...
            // 真實數值來源：聊天消息數（Lv/EXP/鑽石）+ 最早消息時間（認識天數→星星）
            ...
            if (visible.length > 0) {
                ...
                setLastMessage(clean || (last.type === 'image' ? '[圖片]' : '[消息]'));
            } else {
    ...
    // 真實數值：等級/經驗來自聊天消息數（每條 10 exp，升級所需隨等級遞增的三角曲線）；
    // 鑽石 = 聊天消息數本身；星星 = 認識天數（從最早一條消息算起）。
    const stats = useMemo(() => {
        const msgCount = stat.msgCount;
        const totalExp = msgCount * 10;
        const base = 150; // 第 L 級所需經驗 = base×L，累計為三角數
        ...
        const gems = msgCount; // 鑽石 = 聊天消息數本身（不放大）
    ...
    const charName = widgetChar?.name || 'Soren';
    const tagline = (widgetChar?.description || '不知名種草姬').slice(0, 36);
    const announcement = lastMessage || widgetChar?.description || '一切如常，等待新的故事發生。';
    const expPct = Math.min(100, Math.round((stats.exp / stats.expMax) * 100));
    // 時鐘卡角色：優先彼方 chibi 小貼紙（透明立繪），沒有就頭像融合
        ...
        <div className="h-full w-full relative z-10 overflow-hidden select-none" style={{ ...mgVars, color: PAL.ink, fontFamily: FONT_CN }}>
            ...
            <div className="relative h-full overflow-y-auto no-scrollbar px-5"
                ...
                <div className="flex items-center justify-between animate-fade-in">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-bold" style={{ color: PAL.grape, letterSpacing: '0.3em' }}>Soren&nbsp;STATION</span>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => openApp(AppID.Appearance)} aria-label="菜單" className="flex flex-col items-end gap-[3.5px] py-2 active:opacity-60 transition-opacity">
                ...
                <div className="relative mt-4 rounded-3xl overflow-hidden animate-fade-in"
                    ...
                    {/* 角色：有彼方 chibi → 小貼紙；否則頭像電影感融合（參考日常表/攻略本）*/}
                    {chibi?.img && (chibi.isFallback ? (
                        ...
                        <TokenImg value={chibi.img} alt="" loading="lazy"
                            ...
                            style={{
                                height: '64%', // 相對時鐘卡高度，避免吃彼方里按 VR 調的絕對 scale 而巨大
            ...
            <div className="absolute bottom-0 left-0 w-full px-3.5 z-30 pointer-events-none" style={{ paddingBottom: 'calc(var(--safe-bottom, 0px) + 0.75rem)' }}>
                <div className="relative pointer-events-auto rounded-[1.9rem] px-3 py-2.5 flex items-end justify-between"
                    ...
                    {/* 中央羅盤 */}
                    <button onClick={() => setDrawerOpen(true)} aria-label="全部應用" className="-mt-8 w-[4.2rem] h-[4.2rem] rounded-full flex items-center justify-center active:scale-95 transition-transform shrink-0"
                    ...
                    <DockItem id={AppID.Moments} cn="朋友圈" onClick={() => openApp(AppID.Moments)} />
                    <DockItem id={AppID.ThemeMaker} cn="創作" onClick={() => openApp(AppID.ThemeMaker)} />
                </div>
            </div>
            ...
            {drawerOpen && (
                <div className="absolute inset-0 z-40 flex flex-col animate-fade-in" style={{ background: 'var(--mg-drawer)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)' }} onClick={() => setDrawerOpen(false)}>
                    <div className="flex items-center justify-between px-6" style={{ paddingTop: 'calc(var(--safe-top, 0px) + 1.25rem)', paddingBottom: '0.5rem' }}>
                        <h2 className="text-lg tracking-wide" style={{ fontFamily: FONT_CN, color: PAL.grape }}>全部應用</h2>
                        <button onClick={(e) => { e.stopPropagation(); setDrawerOpen(false); }} aria-label="關閉" className="w-9 h-9 rounded-full flex items-center justify-center active:scale-90 transition-transform" style={{ background: 'var(--mg-chip)', border: '1px solid var(--mg-chip-line)' }}>
