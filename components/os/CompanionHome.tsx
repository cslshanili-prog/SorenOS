...
const STARTUP_POSE_CONTROLS: Array<{
  ...
  { key: 'bodyX', label: '身體左右', hint: '左 − / 右 +' },
  { key: 'bodyY', label: '身體俯仰', hint: '後 − / 前 +' },
  { key: 'bodyZ', label: '身體側傾', hint: '右 − / 左 +' },
];

const STARTUP_EMOTION_LABELS: Record<string, string> = {
  neutral: '自然', happy: '開心', sad: '低落', angry: '生氣', fearful: '害怕',
  disgusted: '嫌棄', surprised: '驚訝', calm: '平靜', relaxed: '放鬆',
};

const STARTUP_GESTURE_LABELS: Record<string, string> = {
  idle: '靜止入場', talk: '自然說話', nod: '點頭', shake: '搖頭', tilt: '歪頭',
  explain: '解釋', wave: '揮手', shy: '害羞', 'lean-in': '靠近', 'lean-back': '後退',
};

const STARTUP_FACE_LABELS: Record<string, string> = {
  wink: '眨眼', grin: '咧嘴', pout: '撅嘴', blush: '臉紅', 'eyes-closed': '閉眼',
  'smile-eyes': '笑眼', 'brow-up': '挑眉', 'brow-sad': '憂眉', 'brow-angry': '壓眉',
};
// ── 背景預設：華麗漸變場景（companionBackground = `preset:<id>`）──
...
// ── 打字機：台詞逐字浮現（按真實流逝時間推進；用 interval 而不是 rAF，
// 頁面暫時不合成幀（後台/鎖屏）也能走完，回到前台不會卡在半截）──
...
const CompanionHome: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setHudContent({
        thought: compactCompanionHudText(getLastInnerState(character.id) || scheduleThought, '尚未記錄心聲'),
        recentChat: compactCompanionHudText(latestAssistant?.content, '還沒有聊天記錄'),
        schedule,
      });
    }).catch(() => {
      if (!cancelled) {
        setHudContent({ thought: '', recentChat: '還沒有聊天記錄', schedule: null });
      }
    });
    return () => { cancelled = true; };
  ...
  // ── 主色跟角色走：從頭像提取主色相（跟電子寵物小窩同一套提取器）──
  // 頭像存的是 blobref 令牌，令牌餵給 new Image() 只會靜默加載失敗（取色器 onerror → null），
  // 所以先解析成可加載的 URL 再取色（跟下面背景取色同一套路）。令牌沒解析完時是 undefined，跳過。
  ...
  // ── 背景：preset:<id> / blobref / http 直鏈；空 = 時段天光 ──
  ...
  useEffect(() => {
    // StrictMode 會「裝載→卸載→再裝載」跑一遍 effect：cleanup 把 mounted 打成
    // false 後必須在 effect 體裡設回 true，否則 dev 下開場演出和觸碰回應全被吞。
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setLine({ text, translation: translation || undefined, label: '開場演出', kind: 'startup' });
  ...
  const touchPackContentLabel = activeCompanionSource === 'upload'
    ? '台詞'
    : activeCompanionSource === 'date' ? '台詞與表情' : '台詞與動作';
  const modelActions = useMemo<AvatarTouchModelAction[]>(() => {
    ...
    return vrmExpressions.map(name => ({ id: name, name: `自定義表情：${name}` }));
  ...
  const selectWardrobeAction = (action: Live2DAction) => {
    ...
    addToast(`已手動切換：${action.name}`, 'success');
  };
  ...
  const selectModelOutfit = (assetId: string) => {
    ...
    addToast(`已切換模型：${patch.videoAvatar?.fileName || '當前外觀'}`, 'success');
  };
  ...
  const deleteModelOutfit = async (assetId: string) => {
    ...
    addToast(`${removed.fileName} 已從衣櫥刪除${usedElsewhere ? '（共享模型文件仍保留）' : ''}`, 'success');
  };
  ...
  const deleteStaticOutfit = async (imageRef: string) => {
    ...
    addToast(`${removed.fileName || '圖片 / GIF'} 已從衣櫥刪除${usedElsewhere ? '（共享圖片文件仍保留）' : ''}`, 'success');
  };
  ...
  const deleteWardrobeAction = async (actionId: string) => {
    ...
    addToast(`${action.name} 已從衣櫥移除；動作庫仍保留`, 'success');
  };
      ...
      try {
        if (activeCompanionSource === 'upload') {
          ...
          if (!['png', 'gif'].includes(extension || '') || !['image/png', 'image/gif'].includes(file.type)) {
            throw new Error('圖片衣櫥只支持 PNG / GIF');
          }
          if (file.size > 20 * 1024 * 1024) throw new Error('圖片超過 20 MB，請壓縮後再導入');
          ...
          addToast(`${file.name} 已加入圖片衣櫥`, 'success');
          return;
        }

        const currentModel = character.videoAvatar;
        if (!currentModel) throw new Error('請先設置一個動態模型');
        if (currentModel.format === 'live2d') {
          if (!/\.zip$/i.test(file.name)) throw new Error('Live2D 衣櫥只能繼續導入 Live2D ZIP');
          if (file.size > 200 * 1024 * 1024) throw new Error('Live2D ZIP 超過 200 MB');
          ...
          addToast(`${file.name} 已加入 Live2D 衣櫥，請設置它的換裝按鍵`, 'success');
          return;
        }

        if (!/\.vrm$/i.test(file.name)) throw new Error('VRM 衣櫥只能繼續導入 VRM');
        if (file.size > 80 * 1024 * 1024) throw new Error('VRM 超過 80 MB，請降低紋理尺寸後再導入');
        ...
        closeWardrobe();
        addToast(`${file.name} 已加入 VRM 衣櫥`, 'success');
      } catch (error: any) {
        addToast(error?.message || '衣櫥導入失敗', 'error');
      } finally {
  ...
  const saveCompositionEditor = () => {
    ...
    addToast(
      touchRegionsDraft.length
        ? `角色構圖與 ${touchRegionsDraft.length} 個觸摸圈已保存`
        : faceAnchorDraftEnabled ? '角色構圖與面部特寫錨點已保存' : '角色構圖已保存',
      'success',
    );
  };
  const chooseBuiltinSullyQuality = (quality: BuiltinSullyLive2DQuality) => {
    ...
    addToast(quality === 'hd' ? 'Sully 已切到高清 4K；低端設備建議使用 2K' : 'Sully 已切回輕量 2K', quality === 'hd' ? 'info' : 'success');
  };
  const chooseImportedLive2DTextureQuality = (quality: 'balanced' | 'hd') => {
    ...
    addToast(
      quality === 'hd'
        ? '模型已切到高清 4K；首次切換會建立獨立運行緩存'
        : '模型已切回默認輕量 2K；更省內存、更不易閃退',
  ...
  const selectStartupPreset = (presetId: string) => {
    ...
    addToast(`已切換開場預設「${preset.name}」`, 'success');
  };
  ...
  const selectTouchPreset = (presetId: string) => {
    ...
    touchCursorRef.current = {};
    addToast(`已切換觸摸預設「${preset.name}」`, 'success');
  };
  ...
  const deleteStartupPreset = () => {
    ...
    if (!preset || !window.confirm(`刪除開場預設「${preset.name}」？`)) return;
    const before = companionTouchSettingsBase();
    ...
    setSelectedStartupPresetId('');
    setStartupPresetName('');
    addToast('開場預設已刪除；當前草稿仍保留', 'success');
  };
  ...
  const deleteTouchPreset = () => {
    ...
    if (!preset || !window.confirm(`刪除觸摸預設「${preset.name}」？`)) return;
    const before = companionTouchSettingsBase();
    ...
    setSelectedTouchPresetId('');
    setTouchPresetName('');
    addToast('觸摸預設已刪除；當前反饋仍保留', 'success');
  };
  ...
  const saveStartupSettings = () => {
    ...
    if (startup.enabled && !startup.line) {
      addToast('開啟開場演出前，請先填寫中文原文', 'error');
      return;
    }
    if (startup.enabled && startup.voiceLanguage && !startup.translation) {
      addToast(`已選擇 ${voiceLanguageLabel(startup.voiceLanguage)}，請填寫對應的語音譯文`, 'error');
      return;
    }
    ...
    addToast(`已保存新的開場預設「${saved.preset.name}」`, 'success');
  };
  ...
  const previewStartup = () => {
    ...
    const spokenText = translation || text;
    if (!text) {
      addToast('請先填寫中文原文', 'error');
      return;
    }
    if (startupVoiceLanguage && !translation) {
      addToast(`請填寫 ${voiceLanguageLabel(startupVoiceLanguage)} 語音譯文`, 'error');
      return;
    }
    setTouchSettingsOpen(false);
    setStartupHeadLocked(true);
    setLine({ text, translation: translation || undefined, label: '開場預演', kind: 'startup' });
  ...
  const generateStartupPerformancePack = async () => {
    ...
    try {
      ...
      setSelectedStartupPresetId('');
      addToast(`已按台詞編排 ${cues.length} 個動作拍點；點擊“保存為新預設”後永久複用`, 'success');
    } catch (error: any) {
      if (!mountedRef.current || requestToken !== requestTokenRef.current) return;
      console.warn('[companion] startup performance direction failed:', error);
      addToast(error?.message || '開場動作編排失敗；未保存，也不會重試', 'error');
    } finally {
  ...
  const generateStartupVoicePack = async () => {
    ...
    const translation = normalizeCompanionDialogue(startupTranslation, character.name);
    if (!originalText) {
      addToast('先填寫開場中文原文，再生成語音包', 'error');
      return;
    }
    if (startupVoiceLanguage && !translation) {
      addToast(`請填寫 ${voiceLanguageLabel(startupVoiceLanguage)} 語音譯文`, 'error');
    ...
    if (!characterHasVoice(character, apiConfig)) {
      addToast('這個角色還沒有配置可用音色，請先去語音設置配置', 'error');
      return;
    }
    const requestToken = ++requestTokenRef.current;
    busyRef.current = true;
    ...
    try {
      ...
      setSelectedStartupPresetId('');
      addToast('開場語音包已生成並永久保存在本地；保存為預設後可隨時切換', 'success');
    } catch (error: any) {
      if (!mountedRef.current || requestToken !== requestTokenRef.current) return;
      console.warn('[companion] startup voice pack generation failed:', error);
      addToast(error?.message || '開場語音包生成失敗', 'error');
    } finally {
      if (requestToken === requestTokenRef.current) {
        busyRef.current = false;
        setStartupVoiceGenerating(false);
  ...
  const generateTouchReactionPack = async () => {
    ...
    if (!touchDraftZones.length) {
      addToast('請至少選擇一個可觸摸部位', 'error');
      return;
    }
    if (touchGenerateVoice && !characterHasVoice(character, apiConfig)) {
      addToast('這個角色還沒有配置可用音色，先關閉語音勾選或去語音設置配置', 'error');
      return;
    }
    const requestToken = ++requestTokenRef.current;
    busyRef.current = true;
    ...
    try {
      ...
      const voiceSummary = touchGenerateVoice ? ` · 本地語音 ${voiceGenerated}/${voiceTotal}` : '';
      addToast(`已保存新的觸摸預設「${saved.preset.name}」${voiceSummary}`, 'success');
      if (voiceFailures) {
        addToast(`${voiceFailures} 條語音未能保存，觸摸時只演動作與台詞，不會臨時調用 TTS`, 'info');
      }
    } catch (error: any) {
      if (!mountedRef.current || requestToken !== requestTokenRef.current) return;
      console.warn('[companion] touch reaction pack failed:', error);
      addToast(error?.message || '觸摸反饋包生成失敗', 'error');
    } finally {
      if (requestToken === requestTokenRef.current) {
        busyRef.current = false;
        setTouchGenerating(false);
  ...
  const respondToTouch = (hit: AvatarTouchHit, force = false) => {
    ...
    if (!enabled || !reactions.length) {
      settleAfter(18);
      addToast(`“${avatarTouchZoneLabel(hit.zone)}”還沒有本地反饋，點右側觸摸設置生成一次即可`, 'info');
      return;
    }
    ...
    if (!text) {
      settleAfter(18);
      addToast('這條緩存台詞為空，請在觸摸設置中補生成反饋包', 'error');
      return;
    }
    ...
    touchDialogueTimerRef.current = window.setTimeout(() => {
      ...
      setLine({ text, translation: translation || undefined, label: `觸摸 · ${avatarTouchZoneLabel(hit.zone)}`, kind: 'touch' });
    ...
    return (
      <div className="flex h-full w-full items-center justify-center px-8 text-center text-white/70">
        <div>
          ...
          <div className="text-sm">先創建並選擇一個角色，再來使用觸感陪伴桌面。</div>
          <button onClick={() => openApp(AppID.Character)} className="mt-4 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs">去選擇角色</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative h-full w-full overflow-hidden select-none" data-companion-frame={frameStyle} data-companion-layout="stage" data-wardrobe-hint-active={wardrobeDiscoveryActive ? 'true' : 'false'}>
      ...
      {wardrobeLive2DSettings && (
        <Live2DActionSettings
          ...
          onSave={config => {
            ...
            addToast('Live2D 衣櫥模型已保存', 'success');
          }}
        />
      )}
      ...
      {/* ── 頂部：統一尺寸的角色內容 HUD。真實心聲、聊天和日程直接出現在桌面。 ── */}
          ...
          >
            <header className="flex h-12 items-center justify-between gap-3 px-3 sm:h-14 sm:px-4">
              <button onClick={() => openApp(AppID.Character)} className="flex min-w-0 items-center gap-2 text-left active:scale-[.98]">
                ...
                <span className="min-w-0">
                  ...
                  <span className="block text-[8px] tracking-[0.14em] text-white/50 sm:text-[9px]">{period.label} · {activeCompanionSource === 'upload' ? '圖片 / GIF' : activeCompanionSource === 'date' ? '見面表情同步' : character.videoAvatar ? '動作同步中' : '等待形象'}</span>
                </span>
              </button>
              <div className="flex shrink-0 items-center gap-2">
                <span className="text-[11px] tabular-nums tracking-[0.14em] text-white/75 sm:text-[13px]">{hh}:{mm}</span>
                <button onClick={() => openApp(AppID.Appearance)} className="companion-hud-gear flex h-9 w-9 items-center justify-center border bg-black/20 text-white/80 active:scale-[.96] sm:h-11 sm:w-11" style={{ borderColor: `${uiTint}70` }} aria-label="外觀設置">
                  <Gear size={18} />
                </button>
              </div>
            </header>
            <div className="companion-hud-grid grid grid-cols-3 border-t border-white/10">
              <button onClick={() => openApp(AppID.CheckPhone)} className="min-w-0 border-r border-white/10 px-3 py-2 text-left active:bg-white/5 sm:px-4 sm:py-2.5" data-testid="companion-hud-thought">
                <span className="flex items-center gap-1 text-[8px] font-semibold tracking-[0.14em] sm:text-[9px]" style={{ color: uiTint }}><Sparkle size={11} weight="fill" />當前心聲</span>
                <span className="mt-1 block h-8 overflow-hidden text-[9px] leading-4 text-white/80 sm:h-9 sm:text-[11px]" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{hudContent.thought || '尚未記錄心聲'}</span>
              </button>
              <button onClick={() => openApp(AppID.Chat)} className="min-w-0 border-r border-white/10 px-3 py-2 text-left active:bg-white/5 sm:px-4 sm:py-2.5" data-testid="companion-hud-chat">
                ...
                <span className="mt-1 block h-8 overflow-hidden text-[9px] leading-4 text-white/80 sm:h-9 sm:text-[11px]" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{hudContent.recentChat || '還沒有聊天記錄'}</span>
              </button>
              <button onClick={() => openApp(AppID.Schedule)} className="min-w-0 px-3 py-2 text-left active:bg-white/5 sm:px-4 sm:py-2.5" data-testid="companion-hud-schedule">
                ...
                <span className="mt-1 block h-8 overflow-hidden text-[9px] leading-4 text-white/80 sm:h-9 sm:text-[11px]" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                  {currentScheduleSlot ? `${currentScheduleSlot.emoji || '◌'} ${currentScheduleSlot.startTime} ${currentScheduleSlot.activity}` : '今天還沒有日程'}
                </span>
              </button>
            </div>
          </section>
      ...
      {!editing && !independentChrome && !touchSettingsOpen && !appStarOpen && (
        ...
        >
          ...
          <button onClick={openTouchSettings} className="companion-rail-button group relative z-10 flex flex-col items-center gap-1 active:scale-[.97]" data-testid="companion-touch-settings-button">
            {!preparedReactionCount && <span className="absolute right-0 top-0 z-20 h-1.5 w-1.5 rounded-full bg-[#ff5d9e] ring-2 ring-[#1a1028]" aria-label="尚未生成觸摸反饋" />}
            ...
            <span className="text-[8px] tracking-[0.08em] text-white/95 sm:text-[9px]">觸摸</span>
          </button>
          ...
          >
            ...
            <span className="text-[8px] tracking-[0.08em] text-white/90 sm:text-[9px]">外觀</span>
          </button>
          ...
          <button onClick={openWardrobe} className="companion-rail-button group relative z-10 flex flex-col items-center gap-1 active:scale-[.97]" data-testid="companion-real-wardrobe-button" data-companion-wardrobe-trigger="true">
            ...
            <span className="text-[8px] tracking-[0.08em] text-white/90 sm:text-[9px]">衣櫥</span>
          </button>

          {[
            { id: AppID.Call, icon: 'Call' as const, label: '通話' },
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-[15px] font-semibold tracking-wide text-white">觸摸設置</h2>
                </div>
                <p className="mt-1 max-w-[24rem] text-[10px] leading-relaxed text-white/50">
                  先選可觸摸部位，再一次生成整包反饋。以後每次戳戳只輪播本地台詞和動作，不會逐次調用 API。
                </p>
              </div>
              <button
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[12px] font-semibold tracking-wide text-white">開場演出</h3>
                  ...
                  <div className="mt-1 flex items-center gap-1.5 text-[9px] text-white/48">
                    <span>{startupEnabled ? '已開啟' : '未開啟'} · {startupLine.trim() ? '已填寫開場演出' : '點擊展開設置'}</span>
                    <CaretDown
                      size={11}
              ...
              <div className="mt-3 grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                ...
                >
                  <option value="">{startupPresets.length ? `選擇已保存預設（${startupPresets.length}）` : '還沒有開場預設'}</option>
              ...
              <p className="mt-3 text-[9px] leading-relaxed text-white/48">
                中文原文、語音譯文和動作都由你手動填寫。每次刷新或重啟後演一次；從 App 返回桌面不會重複播放。演出期間暫停隨機轉頭。
              </p>
              ...
              >
                {VOICE_LANGUAGE_OPTIONS.map(option => (
                  <option key={option.value} value={option.value}>{option.value ? option.label : '中文原文（不翻譯）'}</option>
                ))}
              </select>
              ...
              <textarea
                ...
                placeholder={startupVoiceLanguage ? `手動填寫 ${voiceLanguageLabel(startupVoiceLanguage)} 譯文。` : '默認中文時可留空，將直接朗讀上面的中文原文。'}
              ...
              >
                ...
                {startupActionGenerating
                  ...
                  : startupCuesMatchDraft
                    ? `重新編排動作 · 當前 ${startupPerformanceCues.length} 拍`
                    : '讓動作導演按台詞編排'}
              </button>
              <div className="mt-1 text-center text-[7px] leading-relaxed text-white/30">
                一次 LLM 回覆嚴格為每句話生成一個動作；失敗不重試、不兜底。鎖頭只禁止頭部轉動，表情、手臂、身體與專屬動作照常演出。
              </div>

              {startupCuesMatchDraft && (
                <div className="mt-3" data-testid="companion-startup-cue-editor">
                  <div className="mb-1.5 text-[8px] tracking-[0.1em] text-white/46">逐句動作</div>
                  <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar">
                    {startupPerformanceCues.map((cue, index) => {
                      const selected = index === selectedStartupCueIndex;
                      const sentence = startupCueSentences[index]?.text || `動作 ${index + 1}`;
                      return (
                        <button
                          key={`${cue.at}-${index}`}
                          type="button"
                        ...
                        >
                          ...
                          <span className="mt-1 block text-[7px] opacity-55">起始 → {cue.holdMs || 900}ms → {cue.endDirection ? '收尾' : '未設收尾'}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <label className="mt-2 block text-[8px] text-white/46">
                    <span className="flex justify-between"><span>中段保持時長</span><span className="font-mono">{selectedStartupCue?.holdMs || 900}ms</span></span>
                    <input
                      type="range"
                      min={120}
                      max={5000}
              ...
              <input
                ...
                placeholder={`開場演出 ${startupPresets.length + 1}`}
              ...
              <div className="mt-1 text-[7px] leading-relaxed text-white/30">
                保存始終新建一套，不會覆蓋下拉菜單裡的舊預設；已生成語音也隨各自預設獨立保留。
              </div>
              ...
              >
                ...
                {startupVoiceGenerating
                  ? '正在生成並永久保存語音包…'
                  : startupVoiceMatchesDraft ? '重新生成開場語音包' : '生成並永久保存開場語音包'}
              </button>
              <div className="mt-1 text-center text-[7px] leading-relaxed text-white/30">
                {!touchVoiceAvailable
                  ? '角色尚未配置可用音色'
                  : startupVoiceMatchesDraft
                    ? `已保存${savedStartup?.voiceGeneratedAt ? ` · ${new Date(savedStartup.voiceGeneratedAt).toLocaleString()}` : ''}，以後開場直接複用`
                    : savedStartup?.voiceAssetId
                      ? '當前台詞已變化；舊語音仍保存在本地，重新生成後才會播放'
                      : '生成一次後寫入本地語音資產，刷新或重啟不會重新調用 TTS'}
              </div>
              ...
              <details className="mt-3 border-t border-white/10 pt-2" data-testid="companion-startup-precision">
                <summary className="cursor-pointer select-none text-[9px] font-medium tracking-wide text-white/62">
                  {startupCuesMatchDraft ? `精調第 ${selectedStartupCueIndex + 1} 句` : '精調默認動作'} · 眼 / 身體 / 過衝回正
                </summary>
                ...
                <div className="mt-3">
                  <div className="text-[8px] text-white/46">微表情（最多 4 個）</div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-4">
                  <label className="text-[8px] text-white/46">
                    <span className="flex justify-between"><span>輕微過衝</span><span className="font-mono">{Math.round((startupEditorPerformance.precision?.overshoot || 0) * 100)}%</span></span>
                    <input
                      type="range"
                      min={0}
                      max={20}
                  ...
                  <label className="text-[8px] text-white/46">
                    <span className="flex justify-between"><span>回正時長</span><span className="font-mono">{startupEditorPerformance.precision?.settleMs || 920}ms</span></span>
                    <input
                      type="range"
                      min={320}
                      max={2400}
                ...
                <div className="mt-2 text-[7px] leading-relaxed text-white/30">
                  精調只修改當前這一句，不會清空動作編排。開場台詞播放完之前頭部固定正中；身體、手臂、表情和模型專屬動作保持獨立。
                </div>
              </details>
            ...
            <div className="mt-4 grid grid-cols-[minmax(0,1fr)_auto] gap-2">
              <select
                ...
                className="min-w-0 border border-white/12 bg-[#151021] px-3 py-2 text-[10px] text-white/82 outline-none disabled:opacity-45"
              >
                <option value="">{touchPresets.length ? `選擇已保存預設（${touchPresets.length}）` : '還沒有觸摸預設'}</option>
                ...
                return (
                  ...
                  >
                    <span>
                      ...
                      <span className="mt-0.5 block text-[8px] text-white/40">{count ? `已有 ${count} 條` : '尚未生成'}</span>
                    </span>
                    <span
            ...
            <div className="mt-1 text-[7px] leading-relaxed text-white/30">
              反饋包會把中文原文與{touchVoiceLanguage ? ` ${voiceLanguageLabel(touchVoiceLanguage)} ` : '中文'}語音文本分開保存；界面顯示原文，語音只朗讀譯文。
            </div>
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block text-[11px] font-medium text-white/90">同時生成語音</span>
                <span className="mt-0.5 block text-[8px] leading-relaxed text-white/42">
                  {touchVoiceAvailable
                    ? '勾選後預先合成並保存在本地；觸摸時不會臨時調用 TTS'
                    : `角色尚未配置可用音色，當前只生成${touchPackContentLabel}`}
                </span>
              </span>
              <span
            ...
            <input
              ...
              placeholder={`觸摸反饋 ${touchPresets.length + 1}`}
              className="mt-1 w-full border border-white/12 bg-black/15 px-3 py-2 text-[10px] text-white outline-none placeholder:text-white/24 disabled:opacity-45"
            />
            <div className="mt-1 text-[7px] leading-relaxed text-white/30">
              每次生成都會保存為新預設；舊反饋包和它引用的本地語音不會被覆蓋。
            </div>
            ...
            >
              ...
              {touchGenerating
                ? touchVoiceProgress
                  ? `正在合成本地語音 ${touchVoiceProgress.completed}/${touchVoiceProgress.total}…`
            ...
            <div className="mt-2 text-center text-[8px] tracking-wide text-white/30">
              {savedTouchSettings?.generatedAt
                ? `上次生成 ${new Date(savedTouchSettings.generatedAt).toLocaleString()} · 台詞 ${preparedReactionCount} 條 · 語音 ${preparedVoiceCount} 條`
                : `${touchPackContentLabel}正常只請求一次；語音僅在勾選時批量預生成`}
            </div>
          </section>
        </div>
      )}
      {/* ── galgame 對話框：亮色台詞板，不再像聊天消息卡。 ── */}
      {dialogVisible && (
        <div
        ...
        >
          <div className="relative isolate overflow-visible px-4 pb-3 pt-4 text-white" data-testid="companion-dialogue-surface">
            ...
            {line?.kind === 'touch' && lastHit && !thinking && typed.done && (
              ...
              >
                <ArrowClockwise size={11} /> 換一句 · 本地輪播
              </button>
            )}
          </div>
        </div>
            ...
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <div>
                <div className="flex items-center gap-2 text-[13px] font-semibold tracking-[0.16em] text-white">
                  <Sparkle size={15} weight="fill" style={{ color: uiTint }} /> 功能星盤
                </div>
                <div className="mt-0.5 text-[8px] tracking-[0.18em] text-white/35">Soren · 全部真實功能</div>
              </div>
            ...
            <div className="max-h-[calc(52vh-3.4rem)] overflow-y-auto px-3 py-3 no-scrollbar">
              <div className="grid grid-cols-4 gap-x-2 gap-y-3">
                ...
                <button onClick={openTouchSettings} className="flex min-w-0 flex-col items-center gap-1.5 text-white/70 active:scale-90">
                  ...
                  <span className="text-[9px] tracking-wide">觸摸設置</span>
                </button>
              </div>
            </div>
          </section>
        ...
        >
          ...
          <div className="relative z-10 grid h-full grid-cols-5 items-center gap-1 px-2">
            ...
            {[
              ...
              { id: AppID.Settings, icon: Icons.Settings, label: '設置' },
            ].map(item => (
              <button key={item.id} onClick={() => launchCompanionApp(item.id)} className="companion-dock-item flex h-full flex-col items-center justify-center gap-1 text-white/90 active:scale-[.97]">
                <span className="companion-dock-shape flex h-10 w-10 rotate-45 items-center justify-center rounded-[0.7rem] border bg-black/15 sm:h-12 sm:w-12" style={{ borderColor: `${uiTint}72` }}>
                  <item.icon className="companion-dock-icon h-5 w-5 -rotate-45 sm:h-6 sm:w-6" />
      ...
      {/* ── 右側角色檢查器：構圖、裁剪與舞台視覺共用同一個可撤銷編輯流程。 ── */}
      {editing && (
        <>
          {character.videoAvatar && editingPanel === 'character' && (
            ...
            >
              <span className="text-[9px] leading-relaxed text-white/78">拖動角色 · 雙指縮放 · 虛線框為可視區</span>
            </div>
          )}
          <div
          ...
          >
            ...
            >
              <header className="flex items-center justify-between gap-2">
                <div>
                  <div className="flex items-center gap-1.5 text-[12px] font-semibold tracking-[0.12em]"><Crop size={14} weight="bold" style={{ color: uiTint }} />角色構圖</div>
                ...
                <div className="mt-3" data-testid="companion-character-crop-editor">
                  {!character.videoAvatar ? (
                    <div className="rounded-2xl border border-dashed border-white/15 px-4 py-5 text-center">
                      <div className="text-[11px] text-white/70">還沒有可裁剪的視頻角色</div>
                      <button onClick={() => openApp(AppID.Call)} className="mt-2 rounded-full border border-white/15 px-3 py-1.5 text-[10px] text-white/55">去導入 VRM / Live2D</button>
                    </div>
                  ) : (
                    <>
                      {builtinSullyAvatar && (
                        <div className="mb-3 rounded-2xl border border-white/10 bg-black/15 p-2.5" data-testid="companion-builtin-quality-picker">
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="text-[9px] font-semibold tracking-[0.16em] text-white/48">內置模型畫質</div>
                              <div className="mt-0.5 text-[8px] text-white/32">2K 默認更穩；4K 僅在高性能設備使用</div>
                            </div>
                            <span className="text-[8px] text-white/35">{builtinSullyAvatar.builtinQuality === 'hd' ? '≈85 MB' : '≈21 MB'}</span>
                          </div>
                          <div className="mt-2 grid grid-cols-2 gap-1.5">
                            {([
                              { value: 'balanced' as const, label: '輕量 2K' },
                      ...
                      {character.videoAvatar.format === 'live2d' && !builtinSullyAvatar && (
                        <div className="mb-3 rounded-2xl border border-white/10 bg-black/15 p-2.5" data-testid="companion-live2d-texture-quality-picker">
                          <div>
                            <div className="text-[9px] font-semibold tracking-[0.16em] text-white/48">運行紋理畫質</div>
                            <div className="mt-0.5 text-[8px] leading-relaxed text-white/32">默認 2K 更穩；4K 會佔用更多內存，並單獨建立運行緩存</div>
                          </div>
                          <div className="mt-2 grid grid-cols-2 gap-1.5">
                            {([
                              { value: 'balanced' as const, label: '輕量 2K' },
                      ...
                      <div className={`mb-3 grid gap-1.5 ${character.videoAvatar.format === 'live2d' ? 'grid-cols-3' : 'grid-cols-2'}`} data-testid="companion-framing-mode-picker">
                        ...
                        >面部特寫錨點{faceAnchorDraftEnabled ? ' · 已設' : ''}</button>
                        {character.videoAvatar.format === 'live2d' && (
                          ...
                          >觸摸圈選{touchRegionsDraft.length ? ` · ${touchRegionsDraft.length}` : ''}</button>
                        )}
                      </div>
                      {compositionFramingMode === 'face' && (
                        <div className="mb-3 border-l px-2.5 py-2 text-[8px] leading-relaxed text-white/48" style={{ borderColor: `${uiTint}88`, background: `${uiTint}0f` }}>
                          把臉拖到畫面中心並調整到理想大小。保存後，摸臉或 AI 使用「拉近」鏡頭只會落到這個位置，不再按全身比例猜。
                        </div>
                      )}
                      {compositionFramingMode === 'touch' && character.videoAvatar.format === 'live2d' && (
                        <div className="mb-3 rounded-2xl border border-white/10 bg-black/15 p-2.5" data-testid="companion-touch-region-editor-panel">
                          <div className="text-[8px] leading-relaxed text-white/55">
                            先選部位，再在左側模型上按住拖動，圈出橢圓區域。同一部位可畫多個圈；圈會跟隨這個模型，不受半身、全身或構圖縮放影響。
                          </div>
                          <div className="mt-2 grid grid-cols-5 gap-1">
                            {([
                              { zone: 'head', label: '頭', color: '#f5c86a' },
                              { zone: 'face', label: '臉', color: '#ff8fb7' },
                              { zone: 'hand', label: '手', color: '#77d9dd' },
                              { zone: 'body', label: '身體', color: '#9ba8ff' },
                          ...
                          <div className="mt-2 flex items-center justify-between border-t border-white/8 pt-2">
                            <span className="text-[8px] text-white/35">重疊時優先較小的圈</span>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="text-[9px] font-semibold tracking-[0.16em] text-white/48">{compositionFramingMode === 'face' ? '面部錨點大小與位置' : compositionFramingMode === 'touch' ? '圈選時的模型位置' : '大小與位置'}</div>
                        <button
                          onClick={() => {
                        ...
                        >
                          <ArrowClockwise size={10} weight="bold" /> {compositionFramingMode === 'face' ? '重置錨點' : compositionFramingMode === 'touch' ? '重置構圖' : '全部重置'}
                        </button>
                      </div>

                      <label className="mt-2.5 block">
                        <span className="flex items-center justify-between text-[9px] text-white/58"><span>{compositionFramingMode === 'face' ? '特寫大小' : '角色大小'}</span><b className="font-mono text-white/82">{compositionFramingDraft.scale.toFixed(2)}×</b></span>
                      ...
                      <div className="mt-2 flex gap-2">
                        ...
                        {compositionFramingMode === 'face' ? (
                          <button onClick={() => { setFaceAnchorDraftEnabled(false); setCompositionFramingMode('base'); }} className="flex-1 rounded-xl border border-rose-300/20 bg-rose-950/20 py-2 text-[9px] text-rose-200/65 active:scale-[.98]">清除錨點</button>
                        ) : (
                          <button onClick={() => setFramingDraft(defaultCompanionFraming)} className="flex-1 rounded-xl border border-white/12 bg-white/[.045] py-2 text-[9px] text-white/58 active:scale-[.98]">適配舞台</button>
                        )}
                      </div>
                      ...
                      {compositionFramingMode === 'base' && <div className="mt-3 border-t border-white/10 pt-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-[9px] font-semibold tracking-[0.16em] text-white/48">自定義裁剪</div>
                            <div className="mt-0.5 text-[8px] text-white/32">收緊虛線框，隱藏角色畫布的多餘邊緣</div>
                          </div>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2">
                          {([
                            ['top', '上邊'], ['bottom', '下邊'], ['left', '左邊'], ['right', '右邊'],
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-2 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                    <button onClick={() => { void applyCompanionBackground(undefined); }} className={`flex shrink-0 flex-col items-center gap-1 active:scale-95 ${!background ? '' : 'opacity-70'}`}>
                      ...
                      <span className="text-[9px] text-white/60">時段天光</span>
                    </button>
                    ...
                    <button onClick={chooseBackgroundImage} className="flex shrink-0 flex-col items-center gap-1 active:scale-95">
                      ...
                      <span className="text-[9px] text-white/60">{backgroundImageUrl ? '換一張' : '自定義'}</span>
                    </button>
                    {backgroundImageUrl && (
                  ...
                  <div className="mt-4 border-t border-white/10 pt-3" data-testid="companion-frame-style-picker">
                    <div className="text-[9px] tracking-[0.2em] text-white/40">舞台視覺語言</div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="flex h-5 w-5 shrink-0 items-center justify-center border border-white/10 text-white/18" style={active ? { borderColor: `${uiTint}88`, color: uiTint, background: `${uiTint}12` } : undefined}>
                              {active && <Check size={12} weight="bold" aria-label="當前框架" />}
                            </span>
                          </button>
                        );
                      })}
          ...
          {compositionEditorCollapsed && (
            ...
            >
              ...
              <span className="[writing-mode:vertical-rl]">展開構圖</span>
            </button>
          )}
        </>
      )}
