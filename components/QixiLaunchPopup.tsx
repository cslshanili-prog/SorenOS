...
export const QixiLaunchPopup: React.FC<QixiLaunchPopupProps> = ({ onClose }) => {
    ...
    return (
        <div className="qixi-launch-overlay" onMouseDown={event => {
            ...
            >
                <button type="button" className="qixi-launch-close" aria-label="關閉七夕活動提醒" onClick={dismiss}>×</button>
                ...
                <div className="qixi-launch-copy">
                    ...
                    <h2 id="qixi-launch-title"><small>有一條消息</small>掉進了星夜。</h2>
                    <div id="qixi-launch-description">
                        <strong>ta 也正在另一邊找你。</strong>
                        <span>今夜，去「特別時光」選擇一個想見的人。</span>
                    </div>
                </div>

                <footer>
                    <button type="button" className="qixi-launch-primary" onClick={openQixi}>
                        <span>去赴約</span><i aria-hidden="true">✦</i>
                    </button>
                    <button type="button" className="qixi-launch-later" onClick={dismiss}>先把這封信收好</button>
                </footer>

                <p className="qixi-launch-note">活動之後仍可從桌面「特別時光」進入</p>
            </section>
        </div>
    );
};
