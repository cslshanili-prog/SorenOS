...
/**
 * 瑞一杯 使用說明 (首次啟動自動彈一次, 之後收在 banner 的 ? 裡隨時看)
 *
 * 瑞幸 MCP 跟麥當勞邏輯不同: 拉不到整本菜單, 是"告訴角色哪家店的哪杯"由角色去點。
 */
            ...
            <div className="bg-gradient-to-b from-[#FAF7F0] to-[#F3EFE6] w-full sm:max-w-sm rounded-2xl overflow-hidden shadow-2xl flex flex-col" style={{ maxHeight: 'calc(100dvh - var(--safe-top, 0px) - var(--safe-bottom, 0px) - 2rem)' }} onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-[#0B1F3A] to-[#1E4D8C] rounded-t-2xl shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <div className="text-[13px] font-bold text-white">瑞一杯 · 怎麼用</div>
                            <div className="text-[9px] text-white/70">跟麥當勞不太一樣, 花 30 秒看下</div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 text-[12px] text-slate-700 leading-relaxed">
                    ...
                    <div className="bg-white rounded-xl border border-[#E6DFCF] p-3">
                        <div className="text-[12px] font-bold text-[#0B1F3A] mb-1">☕ 直接跟 ta 說"哪家店的哪杯"</div>
                        <div>瑞幸跟麥當勞不一樣——<b>拉不到整本菜單</b>。你直接告訴角色想喝什麼、在哪附近, ta 會自動找最匹配的門店、點好、出微信二維碼掃碼付。</div>
                        <div className="mt-2 bg-[#0B1F3A]/5 rounded-lg px-2.5 py-2 text-[11px] text-[#0B1F3A]">
                            舉例：<br />「幫我點一杯<b>花溪公園附近門店</b>的<b>無糖冰美式</b>」
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        <div><b>不指定門店</b>時, 角色按你<b>當前定位</b>選最近的店。<span className="text-amber-700">開著梯子定位可能不準</span> (啟動時彈的定位框裡能看精度, 不準就手選城市)。</div>
                    </div>

                    {/* 自動選 */}
                    <div className="flex gap-2">
                        <span className="text-[15px] shrink-0">🎯</span>
                        <div><b>不指定飲品 / 杯型 / 冷熱</b>時, 角色會按對你的瞭解<b>自己拿主意</b> (越懂你點得越準, 不滿意直接說"換一杯/要熱的/大杯")。</div>
                    </div>

                    {/* 測試版優惠 */}
                    <div className="flex gap-2">
                        <span className="text-[15px] shrink-0">🧪</span>
                        <div><b>目前是測試版</b>：用不了瑞幸官方的門店優惠, <b>只有你帳號自己的券</b>能自動抵扣。</div>
                    </div>

                    <div className="text-[10px] text-slate-400 text-center pt-1">下單 / 支付都在最後那張「結帳卡」上點, 商品卡只是 ta 給你看的</div>
                </div>

                <div className="border-t border-[#DDD3BC] bg-gradient-to-r from-[#EFE9DC] to-[#E7DFC9] px-3 py-2.5 shrink-0">
                    <button onClick={onClose} className="w-full px-3 py-2.5 bg-[#0B1F3A] text-white text-[13px] font-bold rounded-xl active:scale-95">知道啦, 開點</button>
                </div>
            </div>
        </div>
    );
