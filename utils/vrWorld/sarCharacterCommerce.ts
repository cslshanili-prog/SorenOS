...
export const characterModuleCount = (state: FishingMarketState, actorId: string, moduleId: string) => {
    ...
    if (!Number.isSafeInteger(count) || count < 0) throw new Error('角色倉庫存檔異常，請先備份');
    return count;
};
...
export const acquireCharacterModule = async (actor: MarketActor, moduleId: string, requestId: string, storage: SARStorage = localStorage, now = Date.now()) => {
    if (actor.kind !== 'character' || actor.id === 'user' || !requestId || requestId.length > 160) throw new Error('角色購買信息無效');
    return mutateFishingMarket(current => {
        ...
        if (old) {
            if (old.sarPurchase?.kind !== 'module' || old.sarPurchase.itemId !== moduleId || old.participants[0] !== actor.id) throw new Error('購買記錄不匹配');
            return state;
        }
        ...
        if (characterModuleAllowance(state, actor, now) < module.price) throw new Error('角色今天的零用預算不足，先逛逛也可以');
        return { ...state, accounts: { ...state.accounts, [actor.id]: state.accounts[actor.id] - module.price },
            ...
            ledger: [...state.ledger, { id: requestId, at: now, text: `${actor.name}用自己的 ${module.price} 鱗幣買下「${module.title}」，已放進自己的倉庫。`, participants: [actor.id], deliveredTo: [],
...
export const consumeCharacterModule = (actorId: string, moduleId: string, storage: SARStorage = localStorage) => mutateFishingMarket(current => {
    ...
    if (actorId === 'user') throw new Error('請從自己的模塊袋使用');
    const count = characterModuleCount(state, actorId, moduleId);
    if (!count) throw new Error('角色手裡沒有這枚模塊');
