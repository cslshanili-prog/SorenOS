...
const b={id:'b',name:'旁邊那位',vrState:{enabled:true,intervalMinutes:120}} as any;
const deps={char:a,characters:[a,b],userProfile:{name:'用戶'} as any,apiConfig:{baseUrl:'https://model.invalid/v1',apiKey:'fake',model:'test'} as any,groups:[],updateCharacter:vi.fn(),forcedRoom:'sar' as const,manual:true};
const answer=(text:string)=>vi.mocked(safeFetchJson).mockResolvedValue({choices:[{message:{content:text}}]});
beforeEach(()=>{vi.restoreAllMocks();localStorage.clear();mocks.messages=[];mocks.board={id:'board',messages:[],updatedAt:0};vi.clearAllMocks();saveFishingMarketState({...ensureActorAccounts(createFishingMarketState(42),[{id:'a',name:'艾文',kind:'character'},{id:'b',name:'旁邊那位',kind:'character'},{id:'user',name:'用戶',kind:'user'}]),lastPulseAt:Date.now()});});
it('a board encounter uses one character call and saves the committed scene and reaction in activity cards',async()=>{
    let state=ensureActorAccounts(readFishingMarketState(),[{id:'wanderer:0',name:'臨時售票員',kind:'wanderer'}]);
    state=createRequest(state,{id:'wanderer:0',name:'臨時售票員',kind:'wanderer'},undefined,'替風扇鼓掌',7,'需要一位熱心觀眾',Date.now(),'favor');
    const post=state.requests[0];post.npcPersona={name:'臨時售票員',identity:'風扇的經紀人'};post.encounter={story:'{{participant}}剛站穩，風扇就謝幕了。經紀人說它今天轉得太投入。'};
    saveFishingMarketState(state);
    answer(`<ACTION>fulfill</ACTION><TARGET>${post.id}</TARGET><NOTE>去看看這場演出。</NOTE><REACTION>它的返場是不是得插電？</REACTION>`);
    ...
    expect(cards.some(m=>m.metadata?.marketActivity&&m.content.includes('風扇就謝幕了')&&m.content.includes('返場是不是得插電'))).toBe(true);
...
it('a rejected encounter does not publish the prewritten scene or the character reaction as experienced',async()=>{
    let state=ensureActorAccounts(readFishingMarketState(),[{id:'wanderer:0',name:'臨時售票員',kind:'wanderer'}]);
    state=createRequest(state,{id:'wanderer:0',name:'臨時售票員',kind:'wanderer'},undefined,'替風扇鼓掌',7,'需要觀眾',Date.now(),'favor');
    const post=state.requests[0];post.npcPersona={name:'臨時售票員',identity:'風扇經紀人'};post.encounter={story:'風扇居然謝幕了。'};state.accounts['wanderer:0']=0;
    saveFishingMarketState(state);
    answer(`<ACTION>fulfill</ACTION><TARGET>${post.id}</TARGET><NOTE>去看看。</NOTE><REACTION>沒想到還有返場。</REACTION>`);
    await runVRSession({...deps,forcedSARActivity:'market'});
    expect(mocks.messages.some(m=>m.content.includes('風扇居然謝幕了')||m.content.includes('沒想到還有返場'))).toBe(false);
...
it('manual-only participants cannot be started by a stale timer, but explicit invitations work',async()=>{
    ...
    answer('<NOTE>來看看有什麼。</NOTE><ACTION>browse</ACTION><SHARE>none</SHARE>');
...
it('manual module-shop invitation stays in the shop even when automatic SAR is disabled',async()=>{
    ...
    answer('<ACTIVITY>看了看模塊。</ACTIVITY><NOTE>有點意思。</NOTE><BUY>NO</BUY><USE_ON_USER>NO</USE_ON_USER>');
    ...
    expect(body.messages[0].content).toContain('此刻只在模塊商店');
...
it('manual cabinet invitation cannot randomly turn into fishing',async()=>{
    vi.spyOn(Math,'random').mockReturnValue(0);
    answer(JSON.stringify({title:'芯片測試',story:'一場臨時芯片故事。',notes:'收好隨筆。'}));
...
it('one fishing call keeps the rolled catch, sends actual chat and announces personal unlock',async()=>{
    answer(JSON.stringify({disposition:'keep',reaction:'這條很漂亮。',shareToUser:{text:'快看，我釣到了！'}}));
    ...
    expect(mocks.messages.filter(m=>m.type==='text')).toMatchObject([{charId:'a',content:'快看，我釣到了！'}]);
    expect(mocks.messages.some(m=>m.charId==='b'&&m.content.includes('首次解鎖'))).toBe(true);
    const payload=JSON.parse((vi.mocked(safeFetchJson).mock.calls[0][1] as any).body);expect(payload.messages[0].content).toContain('ORIGINAL PERSONA');expect(payload.messages.at(-1).content).toContain('程序判定的唯一魚獲');
...
it('release and share are independent; duplicate species never repeats unlock announcements',async()=>{
    vi.spyOn(Math,'random').mockReturnValue(0);
    for(let i=0;i<2;i++){
        answer(JSON.stringify({disposition:'release',reaction:'小魚回家吧。',shareToUser:{text:'放回去了。'}}));
...
it('sells to Aiven at the end of one fishing call and retries delivery without another payment',async()=>{
    vi.spyOn(Math,'random').mockReturnValue(0);
    answer(JSON.stringify({disposition:'sell',reaction:'今天有收穫。',saleWords:'這條你收嗎？',shareToUser:{text:'把魚賣給艾文了。'}}));
    expect((await runVRSession({...deps,forcedSARActivity:'fishing'})).ok).toBe(true);
    ...
    expect(card.content).toContain('艾文的成交回應');expect(card.content).toContain('這條你收嗎？');
    expect(mocks.messages.filter(m=>m.type==='text')).toMatchObject([{content:'把魚賣給艾文了。'}]);
...
it('invalid JSON retains pending catch, and a model cannot release clay or invoke market',async()=>{
    ...
    answer(JSON.stringify({disposition:'market',reaction:'賣了',shareToUser:null}));
    expect((await runVRSession({...deps,forcedSARActivity:'fishing'})).ok).toBe(false);
...
it('completed tip reaches both characters and the next call can react to what actually happened',async()=>{
    let s=readFishingMarketState();s=createRequest(s,{id:'b',name:'旁邊那位',kind:'character'},undefined,'給我錢',30,'給我錢！！',Date.now(),'tip');saveFishingMarketState(s);
    answer(`<NOTE>還真敢要錢。</NOTE><ACTION>fulfill</ACTION><TARGET>${s.requests[0].id}</TARGET><WORDS>拿好</WORDS><SHARE>none</SHARE>`);
    ...
    expect(mocks.messages.some(m=>m.charId==='b'&&m.content.includes('真的給')&&m.content.includes('給我錢！！'))).toBe(true);
    answer('<NOTE>居然真有人給了！</NOTE><ACTION>browse</ACTION><SHARE>guestbook</SHARE><SHARE_WORDS>我要了30塊，居然真收到了！</SHARE_WORDS>');
    expect((await runVRSession({...deps,char:b,forcedSARActivity:'market'})).ok).toBe(true);
    const body=JSON.parse((vi.mocked(safeFetchJson).mock.calls[1][1] as any).body);expect(body.messages.at(-1).content).toContain('打賞了 30');
    expect(vi.mocked(buildChatRequestPayload).mock.calls[1][0].historyMsgs.some(m=>m.content.includes('打賞了 30'))).toBe(true);
...
it('invalid action does not pay or broadcast an invented success',async()=>{
    answer('<NOTE>我要買</NOTE><ACTION>buy</ACTION><TARGET>nonexistent</TARGET><SHARE>guestbook</SHARE><SHARE_WORDS>我已買走整個世界！</SHARE_WORDS>');
...
it('preserves original no-water room prompt behavior and does not initialize fishing storage',async()=>{
    ...
    expect(localStorage.getItem('vr_fishing_market_v1')).toBeNull();const payload=JSON.parse((vi.mocked(safeFetchJson).mock.calls[0][1] as any).body);expect(payload.messages[0].content).not.toContain('鱗幣');expect(payload.messages[0].content).not.toContain('佈告板');
});
...
it('garden uses the real persona flow for one grid action and saves its factual event',async()=>{
    const user={id:'user',name:'用戶',kind:'user' as const};let s=setGardenVisits(ensureDinosaurGarden(readFishingMarketState(),user),true,user);
    const id=gardenResidents(s)[0].catchId,spot=findGardenSpace(s,'new');saveFishingMarketState(s);
    answer(JSON.stringify({action:'move',toyId:id,slotId:spot.slotId,words:'去橋邊等吧。'}));
    ...
    expect(mocks.messages.some(m=>m.content.includes('去橋邊等吧。'))).toBe(true);
    const body=JSON.parse((vi.mocked(safeFetchJson).mock.calls[0][1] as any).body);expect(body.messages[0].content).toContain('ORIGINAL PERSONA');expect(body.messages.at(-1).content).toContain('隱藏棋盤');expect(body.messages.at(-1).content).not.toContain('錢包');
});
it('garden rejects malformed output without a fake visit, and disabled co-editing does not call a model',async()=>{
    const user={id:'user',name:'用戶',kind:'user' as const};let s=ensureDinosaurGarden(readFishingMarketState(),user);saveFishingMarketState(s);
    ...
    answer('我已經把整張桌子賣掉了！');expect((await runVRSession({...deps,forcedSARActivity:'garden'})).ok).toBe(false);
