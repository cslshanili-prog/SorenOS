...
export const remainingSARBuyback = (budgets: Record<string, SARBuybackBudget> | undefined, actorId: string, at = Date.now()) => {
    ...
    if (!Number.isSafeInteger(budget.earned) || budget.earned < 0 || typeof budget.day !== 'string') throw new Error('回收額度存檔異常，請先備份');
...
export const creditSARWallet = (balance: number, amount: number) => {
    if (!Number.isSafeInteger(balance) || balance < 0 || !Number.isSafeInteger(amount) || amount < 0) throw new Error('錢包金額異常，請先備份');
    if (amount === 0) return balance;
    if (balance + amount > SAR_WALLET_LIMIT) throw new Error(`錢包最多可存 ${SAR_WALLET_LIMIT.toLocaleString()} 鱗幣，請先花一些再收款`);
