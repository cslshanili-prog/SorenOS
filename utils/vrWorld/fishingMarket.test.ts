...
describe('local fishing economy',()=>{
    ...
    it('retains names of past visitors and handles missing legacy names without displaying IDs',()=>{
        ...
        expect(M.marketActorName([],'deleted-character','來訪的朋友')).toBe('來訪的朋友');
        expect(M.marketActorName([],undefined)).toBe('未記錄姓名');
    });
    ...
    it.each([['雷陣雨','storm'],['小雪','snow'],['rain','rain'],['陰','cloudy'],['haze','fog'],['晴','clear']])('maps weather %s', (source,kind)=>expect(M.weatherFromDescription(source)).toBe(kind));
    ...
    it('uses enabled real perception and visibly falls back if that source is unavailable',async()=>{
        ...
        weatherFetch.mockResolvedValueOnce({city:'上海',description:'雷陣雨',temp:25});
        expect(await M.resolveFishingWeather(config,42)).toMatchObject({kind:'storm',source:'real',detail:'上海 · 雷陣雨 · 25°C'});
        expect(weatherFetch).toHaveBeenCalledWith(config);
        weatherFetch.mockRejectedValueOnce(Error('offline'));expect(await M.resolveFishingWeather(config,42)).toMatchObject({source:'simulated',detail:'真實天氣暫不可用 · 使用彼方今日天氣'});
    });
    it('transfers real inventory and conserves money, without repeat purchase',()=>{
        const caught=fish();let s=M.createListing(M.addCatchToState(init(),caught),a,caught,123,'我壟斷全宇宙',now);
        ...
        expect(s.ledger.at(-1)?.participants.sort()).toEqual(['a','b']);expect(s.ledger.at(-1)?.quotes?.[0].content).toBe('我壟斷全宇宙');
    ...
    it('zero price and imaginary goods are allowed, no imaginary inventory is minted',()=>{
        let s=M.createListing(init(),a,null,0,'你們到底想幹嘛！！',now,'一個響指','匿名7');
        s=M.buyListing(s,s.listings[0].id,b,now);expect(s.accounts).toEqual(init().accounts);expect(s.inventory).toHaveLength(0);
        expect(s.ledger.at(-1)?.text).toContain('僅文字約定');expect(s.ledger.at(-1)?.text).toContain('匿名7');
    });
    it.each([-1,.5,NaN,Infinity,1000001])('rejects invalid price %s',n=>expect(()=>M.createListing(init(),a,null,n,'',now,'空氣')).toThrow());
    it('rejects self purchase, missing goods and insufficient balance without changes',()=>{
        ...
        expect(()=>M.buyListing(s,s.listings[0].id,b,now)).toThrow('餘額');expect(()=>M.buyListing(s,s.listings[0].id,a,now)).toThrow('自己');
        expect(()=>M.buyListing({...s,inventory:[]},s.listings[0].id,b,now)).toThrow('沒有');expect(JSON.stringify(s)).toBe(snapshot);
    });
    it('comments never pay, tips pay responder to poster and retain full archive',()=>{
        let s=M.createRequest(init(),a,undefined,'給我錢',50,'我是宇宙之王',now,'tip','神秘人');const id=s.requests[0].id;
        s=M.commentOnPost(s,id,b,'？？？','交易員',now);expect(s.accounts.a).toBe(1000);
    ...
    it('item requests need owned unlisted inventory; favors pay only after text submitted',()=>{
        let s=M.createRequest(M.addCatchToState(init(),fish(b)),a,'glass-minnow','想要魚',20,'',now);const id=s.requests[0].id;
        expect(()=>M.fulfillRequest(s,id,user,'我說我有',now)).toThrow();
        s=M.fulfillRequest(s,id,b,'請收好',now);expect(s.inventory[0].ownerId).toBe('a');expect(s.accounts.a).toBe(980);
        s=M.createRequest(s,a,undefined,'一句鼓勵',0,'',now,'favor');const favor=s.requests.at(-1)!.id;
        expect(()=>M.fulfillRequest(s,favor,b,'',now)).toThrow();expect(M.fulfillRequest(s,favor,b,'會好起來的',now).requests.at(-1)?.status).toBe('fulfilled');
    });
    it('expiry and deletion archive without erasing comments and unlock the fish',()=>{
        ...
        s=M.commentOnPost(s,id,b,'原回覆','',now);expect(M.availableCatches(s,'a',now)).toHaveLength(0);
        expect(()=>M.buyListing(s,id,b,now+M.MARKET_DAY_MS)).toThrow();expect(()=>M.commentOnPost(s,id,b,'晚到','',now+M.MARKET_DAY_MS)).toThrow();
        const expired=M.ensureMarketDay(s,now+M.MARKET_DAY_MS);expect(expired.listings[0]).toMatchObject({status:'expired',note:'原文',comments:[{content:'原回覆'}]});expect(M.availableCatches(expired,'a',now+M.MARKET_DAY_MS)).toHaveLength(1);
    ...
    it('keeps the post owner anonymous when replying without a new alias',()=>{
        let s=M.createListing(init(),a,null,0,'原文',now,'一句晚安','月亮交易員');
        s=M.commentOnPost(s,s.listings[0].id,a,'還在哦','',now);
        expect(s.listings[0].comments[0].alias).toBe('月亮交易員');
        expect(s.ledger.at(-1)?.text).not.toContain(a.name);
        expect(s.ledger.at(-1)?.quotes?.[0].name).toBe('月亮交易員');
        s=M.createRequest(s,a,undefined,'一句鼓勵',0,'',now,'favor','匿名發帖人');
        s=M.commentOnPost(s,s.requests[0].id,a,'謝謝','',now);
        expect(s.requests[0].comments[0].alias).toBe('匿名發帖人');
    ...
    it('delivers the chosen specimen and archives it without taking another of the same species',()=>{
        ...
        s=M.createRequest(s,a,'glass-minnow','求魚',20,'',now);
        expect(()=>M.fulfillRequest(s,s.requests[0].id,b,'',now)).toThrow('選擇');
        s=M.fulfillRequest(s,s.requests[0].id,b,'這條給你',now,'chosen');
    ...
    it('keeps the listed specimen details in the archive even after it leaves inventory',()=>{
        let s=M.createListing(M.addCatchToState(init(),fish(a)),a,fish(a),20,'實物',now);
