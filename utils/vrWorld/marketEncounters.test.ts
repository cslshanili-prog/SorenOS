...
const user: M.MarketActor = {id:'user',name:'用戶',kind:'user'};
const char: M.MarketActor = {id:'char',name:'朋友',kind:'character'};
const story = '{{participant}}剛進門，報幕員就宣佈電梯抵達三樓。門外仍是一樓。他小聲解釋：這是配音演員的期末補考。';
...
function fixture(mode: 'buy'|'work'|'free' = 'buy', price = 16) {
 ...
 const data={personas:ids.map((actorId,i)=>({actorId,name:'路人'+i,identity:'電梯配音專業的留級生'})),actions:[
  {actorId:ids[0],action:'encounter',ref:'n1',mode,price,title:'有償試乘紙箱電梯',words:'樓層不保真，報站很認真。',event:{story}},
  {actorId:ids[1],action:'comment',targetId:'n1',words:'你昨天還說自己是貨梯。'},
  {actorId:ids[0],action:'comment',targetId:'n1',words:'專業調劑了。'},
 ]};
...
it('generates public posts and private complete events together, with a temporary persona and no execution',()=>{
 ...
 expect(next.prompt).not.toContain('期末補考');expect(next.visitors[0].persona?.name).toBe('路人0');
...
it('a user purchase reveals the prewritten event once, debits only the posted amount and survives backup',()=>{
 ...
 expect(after.listings[0].encounterResult?.story).toBe(story.replace('{{participant}}','用戶'));
 expect(after.inventory).toHaveLength(0);expect(()=>M.buyListing(after,post.id,user)).toThrow();
 expect(marketReceiptContent(after.ledger.at(-1)!)).toContain('期末補考');
 ...
 expect(prepareMarketNPCs(after,()=>.1).prompt).toContain('期末補考');
});
...
it('a character chooses one post and its reaction lands with that exact event in the receipt',()=>{
 ...
 const p=parseMarketPlan(`<ACTION>fulfill</ACTION><TARGET>${post.id}</TARGET><NOTE>去搭一下這個電梯。</NOTE><REACTION>下次能報個博士樓層嗎？</REACTION>`)!;
 const after=applyMarketPlan(state,char,p);
 expect(after.requests[0].encounterResult).toMatchObject({participantId:'char',reaction:'下次能報個博士樓層嗎？'});
 expect(after.ledger.at(-1)?.text).toContain('下次能報個博士樓層嗎');
 expect(after.ledger.at(-1)?.text).toContain('朋友剛進門');
});
it('failed/expired transactions and mere comments never reveal an event or record a reaction',()=>{
 ...
 expect(()=>M.buyListing({...state,accounts:{...state.accounts,user:0}},post.id,user,Date.now(),'反應')).toThrow();
 ...
 expect(commented.ledger.at(-1)?.text).not.toContain('期末補考');
});
...
it('refuses to rename an NPC with open posts, and changing persona never refills a wallet',()=>{
 ...
 const altered=structuredClone(data);altered.personas[0].name='另一個人';expect(()=>parseMarketNPCs(JSON.stringify(altered),snapshot)).toThrow();
...
it('other NPCs cannot consume hidden encounters intended for the user or characters',()=>{
 ...
 const result=applyMarketNPCs(state,snapshot,[{actorId:'wanderer:1',action:'buy',targetId:post.id,words:'代買。'},{actorId:'wanderer:0',action:'comment',targetId:post.id,words:'等等。'}]);
