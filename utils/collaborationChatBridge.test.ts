...
describe('協同實時私聊與選擇轉發',()=>{
  it('10/20/跟隨範圍每輪重新讀庫，遵守用戶斷點，不讀其他角色和群聊',async()=>{
    const char={id:'bridge-char',name:'測試',contextLimit:100,contextRangeMode:'manual',contextRangePolicyVersion:1} as CharacterProfile;
    await DB.saveCharacter(char);
    const ids:number[]=[];
    for(let i=0;i<25;i++)ids.push(await DB.saveMessage({charId:char.id,role:'user',type:'text',content:'要求'+i}));
    await DB.saveMessage({charId:'other',role:'user',type:'text',content:'別人'});
  ...
  it('只轉發選中且屬於本窗口的正文和附件，保留順序，不包含思考過程',()=>{
    const messages:CollaborationMessage[]=[
      {id:'one',sessionId:'a',role:'user',content:'第一條',createdAt:1},
      {id:'two',sessionId:'a',role:'assistant',content:'第二條',createdAt:2,thinkingChain:'不要轉發思考'},
    ...
    expect(selected).toEqual([{role:'assistant',type:'text',content:'第二條',timestamp:2}]);
    expect(selectCollaborationTransfer(messages,'a',new Set(['two','one'])).map(m=>m.content)).toEqual(['第一條','第二條']);
  });
});
