...
const presets: Record<string, ChatTheme> = {
    default: {
        id: 'default', name: '默認', type: 'preset',
...
describe('resolveChatTheme', () => {
    ...
    it('custom 同 id 覆蓋 preset', () => {
        const custom = { ...presets.dream, id: 'dream', name: 'DIY夢境', type: 'custom' as const };
        expect(resolveChatTheme('dream', [custom], presets).name).toBe('DIY夢境');
    });

    it('legacy 主題缺 user/ai 側時用 default 補全（不炸 styleConfig 讀取）', () => {
        const broken = { id: 'old', name: '舊', type: 'custom' } as unknown as ChatTheme;
