...
export const LAYOUT_DEFAULTS = {
 ...
 chatHeaderAlign:'left',chatHeaderDensity:'default',chatStatusStyle:'subtle',chatSendButtonStyle:'circle',chatHideHeaderBuffs:false,
...
export function pickDecorationLayout(value:unknown):DecorationLayout{
 if(!record(value))throw Error('佈局內容無效');const result:Record<string,unknown>={};
 for(const [key,def] of Object.entries(LAYOUT_DEFAULTS))if(value[key]!==undefined){const v=value[key];
  if(typeof v!==typeof def||choices[key]&&!choices[key].includes(v)||typeof v==='number'&&(!Number.isFinite(v)||Math.abs(v)>200))throw Error('佈局選項無效：'+key);
  result[key]=v;
...
export const PART_LABELS={layout:'佈局',bubbles:'氣泡',background:'背景',sound:'聲音',css:'進階 CSS'} as const;
...
function resource(value:unknown):string{
 if(typeof value!=='string'||value.length>40*1024*1024||!(/^(data:(image|audio)\/[\w.+-]+[;,]|https?:\/\/)/i.test(value)))throw Error('資源不是可分享的圖片或音頻，請重新導出原文件');return value;
}
export function validateBubble(value:unknown):ChatTheme{
 if(!record(value)||typeof value.name!=='string'||!record(value.user)||!record(value.ai))throw Error('氣泡主題格式無效');
 ...
 if(value.customCss!==undefined){if(typeof value.customCss!=='string')throw Error('氣泡 CSS 無效');result.customCss=value.customCss;}
 return result;
}
export function validateDecoration(value:unknown):DecorationPreset{
 if(!record(value)||value.format!=='sullyos-chat-decoration')throw Error('不是裝扮預設');if(value.version!==1)throw Error('此裝扮版本暫不支持，請更新應用');
 if(!record(value.parts))throw Error('裝扮缺少內容');const p=value.parts,result:DecorationPreset={format:'sullyos-chat-decoration',version:1,name:typeof value.name==='string'?value.name.slice(0,60):'導入的裝扮',parts:{}};
 ...
 if(p.background!==undefined){if(!record(p.background)||!['plain','grid','paper','mesh'].includes(p.background.style))throw Error('背景格式無效');result.parts.background={image:p.background.image===null?null:resource(p.background.image),style:p.background.style};}
 if(p.sound!==undefined){if(p.sound===null)result.parts.sound=null;else{if(!record(p.sound)||typeof p.sound.src!=='string')throw Error('提示音格式無效');const src=p.sound.src;if(src!=='none'&&!Object.hasOwn(BUILTIN_SOUNDS,src))resource(src);if(p.sound.volume!==undefined&&(typeof p.sound.volume!=='number'||p.sound.volume<0||p.sound.volume>1))throw Error('音量無效');result.parts.sound={src,volume:p.sound.volume??.6};}}
 if(p.css!==undefined){if(typeof p.css!=='string'||p.css.length>8*1024*1024||/<\/?(?:script|html|iframe)\b/i.test(p.css))throw Error('CSS 內容無效');result.parts.css=p.css;}
 if(!Object.keys(result.parts).length)throw Error('預設中沒有可用的裝扮內容');
 return result;
}
async function portable(value:string):Promise<string>{
 if(value.startsWith('blobref:')){const result=await resolveRefToDataUrl(value);if(!result||result.startsWith('blobref:'))throw Error('部分素材已丟失，請重新選擇後導出');return result;}
 if(/^(blob:|\/|\.\.?\/)/.test(value)){const response=await fetch(value);if(!response.ok)throw Error('素材無法讀取');return blobToDataUrl(await response.blob());}
 return value;
}
...
export async function exportDecoration(name:string,theme:OSTheme,char:CharacterProfile|undefined,bubble:ChatTheme):Promise<DecorationPreset>{
 ...
 const result=validateDecoration(await portableTree(preset));if(new Blob([JSON.stringify(result)]).size>40*1024*1024)throw Error('整套素材超過 40 MB，請精簡背景或氣泡圖片後導出');return result;
}
export type DecorationImport={kind:'preset';preset:DecorationPreset}|{kind:'image';image:string;name:string};
export function parseDecorationText(text:string,name='導入的裝扮'):DecorationPreset{
 const source=text.replace(/^\uFEFF/,'').trim().replace(/^```(?:css)?\s*\n([\s\S]*?)\n```$/i,'$1').trim();
 if(source.startsWith('SULLYSND1:')){const sound=decodeSoundShare(source);if(!sound)throw Error('聲音分享碼無效');return validateDecoration({format:'sullyos-chat-decoration',version:1,name,parts:{sound}});}
 if(/^[\[{]/.test(source)){let v;try{v=JSON.parse(source);}catch{if(source.startsWith('{'))throw Error('文件格式不完整，無法導入');}if(v!==undefined){if(v?.format==='sullyos-chat-decoration')return validateDecoration(v);if(v?.user&&v?.ai)return validateDecoration({format:'sullyos-chat-decoration',version:1,name:v.name||name,parts:{bubbles:v}});throw Error('這份文件不是裝扮、氣泡或 CSS，請到對應功能導入');}}
 if(!source||!/[{}]/.test(source)||!/[\w-]+\s*:[^{}]+[;}]/.test(source)||/<\/?(?:html|script|style|iframe)\b/i.test(source))throw Error('沒有識別到 CSS。請選擇 CSS/TXT 原文件，或裝扮預設');
...
export async function readDecorationFile(file:File):Promise<DecorationImport>{
 if(file.size>40*1024*1024)throw Error('文件超過 40 MB，請精簡素材後重試');
 const bytes=new Uint8Array(await file.arrayBuffer());
 if(isPng(bytes)&&pngHasShare(bytes)){const {metadata,payload}=extractShareFromPng(bytes);if(!['chat-decoration','chrome-css','chat-theme','whitebox-sound'].includes(metadata.kind))throw Error(`這是${SHARE_KINDS[metadata.kind]}，請到對應功能導入`);return {kind:'preset',preset:parseDecorationText(new TextDecoder().decode(payload),metadata.title)};}
