...
const weather={kind:'clear',label:'晴',detail:'彼方天氣',source:'simulated'} as const;
...
it('simple fishing animates casting and reeling, saves once, and blocks duplicate casts',async()=>{
    ...
    act(()=>button('簡單').click());const cast=button('拋竿');act(()=>{cast.click();cast.click();});
    ...
    await act(async()=>finish());expect(button('再釣一次').disabled).toBe(false);
...
it('retries the same catch after storage failure and blocks rerolling it away',async()=>{
    ...
    const onCaught=vi.fn().mockRejectedValueOnce(new Error('寫入失敗')).mockResolvedValue(undefined),onCast=vi.fn(()=>catchData);
    await act(async()=>root.render(React.createElement(FishingGame,{weather,onCast,onCaught})));
    await act(async()=>button('拋竿').click());await act(async()=>(window as any).advanceTime(4300));expect(container.querySelector('[role=alert]')?.textContent).toContain('寫入失敗');
    expect(button('再釣一次').disabled).toBe(true);await act(async()=>button('重試收魚').click());
...
it('cancels a manual cast without saving and suppresses the water context menu',async()=>{
    ...
    act(()=>button('拋竿').click());expect(button('簡單').disabled).toBe(true);act(()=>button('收竿').click());expect(onCaught).not.toHaveBeenCalled();
...
it('simple fishing can return empty and never saves an unearned catch', async()=>{
    ...
    await act(async()=>root.render(React.createElement(FishingGame,{weather,onCast,onCaught})));
    act(()=>button('拋竿').click());await act(async()=>(window as any).advanceTime(4300));
    expect(JSON.parse((window as any).render_game_to_text()).phase).toBe('escaped');
    expect(container.textContent).toContain('空軍');expect(onCaught).not.toHaveBeenCalled();expect(onCast).toHaveBeenCalledTimes(1);
});
