...
const dialogue=(ids:string[])=>[{actorId:ids[0],action:'post',ref:'n1',title:'魚販請回答',words:'隔壁昨天說魚會自己砍價，現在還作數嗎？'},...ids.slice(1).map(actorId=>({actorId,action:'comment',targetId:'n1',words:'你先讓魚開口，我負責記帳。'})),{actorId:ids[0],action:'comment',targetId:'n1',words:'它剛吐的那個泡算不算口頭報價？'}];
...
describe('佈告板多人路人',()=>{
 ...
 it('preserves pending user posts and uses generated actual replies to existing targets',()=>{
  const input=createRequest(initial(),user,undefined,'求問',0,'真的能砍價嗎？',Date.now(),'favor'),snapshot=prepareMarketNPCs(input,()=>.1);
  const actions=dialogue(snapshot.visitors.map(v=>v.id));actions[0]={actorId:'wanderer:0',action:'comment',targetId:input.requests[0].id,words:'看魚願不願意。'} as any;actions.forEach(a=>{if('targetId' in a)a.targetId=input.requests[0].id;});
  const state=applyMarketNPCs(input,snapshot,plan(snapshot,actions)).state;expect(state.requests[0].body).toBe('真的能砍價嗎？');expect(state.requests[0].comments).toHaveLength(3);expect(snapshot.prompt).toContain('真的能砍價嗎');
 });
 it('rejects impersonation, bad refs, excessive actions and multiple transactions before any write',()=>{
  ...
  expect(()=>parseMarketNPCs('隨口一說',snapshot)).toThrow();
 });
 it('rechecks funds and keeps unrelated fresh edits when a purchase can no longer happen',()=>{
  const input=createListing(initial(),user,null,100,'出售空氣',Date.now(),'一袋空氣'),snapshot=prepareMarketNPCs(input,()=>.1);
  const actions=plan(snapshot,[{actorId:'wanderer:0',action:'buy',targetId:input.listings[0].id,words:'我看看夠不夠錢。'} as any,{actorId:'wanderer:1',action:'comment',targetId:input.listings[0].id,words:'我圍觀。'} as any,{actorId:'wanderer:0',action:'comment',targetId:input.listings[0].id,words:'算了再看看。'} as any]);
...
describe('路人一次模型調用',()=>{
 ...
 it('missing API, API failure and malformed outputs never fall back to fixed posts',async()=>{
  ...
  vi.mocked(safeFetchJson).mockResolvedValueOnce({choices:[{message:{content:'我想說句話'}}]});await expect(runMarketNPCSession(api)).rejects.toThrow('格式');expect(localStorage.getItem('vr_fishing_market_v1')).toBe(before);
 });
 it('leaving or switching off NPCs during generation prevents writeback',async()=>{
  ...
  vi.mocked(safeFetchJson).mockImplementation(async(_url,options)=>{controller.abort();return replyFor(options);});await expect(runMarketNPCSession(api,controller.signal)).rejects.toThrow('離開');expect(localStorage.getItem('vr_fishing_market_v1')).toBe(before);
  vi.mocked(safeFetchJson).mockImplementation(async(_url,options)=>{localStorage.setItem('vr_sar_club_state_v1','{"npcPreference":"hide"}');return replyFor(options);});await expect(runMarketNPCSession(api)).rejects.toThrow('關閉');expect(localStorage.getItem('vr_fishing_market_v1')).toBe(before);
 });
