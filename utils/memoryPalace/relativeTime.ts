...
const numberPattern = '[0-9零〇一二兩三四五六七八九十百千]+';
const expressionPattern = `昨天|昨晚|前天|今天|明天|後天|上週|上個月|去年|${numberPattern}天前|${numberPattern}個月前`;
...
function expressions(content: string) {
    ...
    return [...content.matchAll(new RegExp(expressionPattern, 'g'))].filter(match => {
        ...
        if (/[\d零〇一二两兩三四五六七八九十百千万萬点點.]/.test(before)) return false;
        if (before === '大' && /^(前天|[后後]天)$/.test(match[0])) return false;
        if (match[0] === '上週' && /^[一二三四五六日天末]/.test(after)) return false;
...
export function relativeTimeAnnotations(node: MemoryNode, enabled: boolean): RelativeTimeAnnotation[] {
    ...
    if (!anchor) return [];
    const dayOffsets: Record<string, number> = { 昨天: -1, 昨晚: -1, 前天: -2, 今天: 0, 明天: 1, 後天: 2, 上週: -7 };
    return expressions(node.content).flatMap(match => {
        const expression = match[0];
        let label: string;
        if (expression in dayOffsets) {
            label = chineseDate(addLocalDays(key, dayOffsets[expression])) + (expression === '上週' ? '前後' : '');
        } else if (expression === '去年') {
            label = `${anchor.getFullYear() - 1}年`;
        } else {
            const months = expression === '上個月' || expression.endsWith('個月前');
            const amount = expression === '上個月' ? 1 : readNumber(expression.replace(months ? /[个個]月前$/ : /天前$/, ''));
