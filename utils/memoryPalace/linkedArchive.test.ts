...
describe('new linked archives, old archives unchanged', () => {
    const old = { id: 'old', date: '2026-09-16', summary: '舊版文本', mood: 'palace' };
    const node = { id: 'node', charId: 'char', content: '新記憶', createdAt: new Date(2026, 8, 16, 12).getTime() };
    it('adds one explicit link per new node and does not merge into old same-day snapshots', () => {
        ...
        expect(merged[1]).toMatchObject({ palaceMemoryId: 'node', summary: '新記憶', date: old.date });
    ...
    it('resolves only explicit links and leaves fallback text untouched in storage', async () => {
        const spy = vi.spyOn(MemoryNodeDB, 'getById').mockResolvedValue({ ...node, content: '宮殿修改後' } as MemoryNode);
        try {
            const linked = buildAutoArchiveFragments([node], 0, true)!.fragments[0];
            const resolved = await resolveLinkedArchives('char', [old, linked]);
            expect(resolved[0]).toBe(old); expect(resolved[1].summary).toBe('宮殿修改後');
            expect(linked.summary).toBe('新記憶'); expect(spy).toHaveBeenCalledTimes(1);
