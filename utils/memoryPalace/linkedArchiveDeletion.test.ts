...
async function seed(id: string) {
    const node: MemoryNode = { id, charId: `char-${id}`, content: '宮殿後來修改的內容', createdAt: 1, room: 'living_room', tags: [], importance: 5, mood: '', embedded: false, lastAccessedAt: 1, accessCount: 0 };
    const memories: MemoryFragment[] = [
        ...
        { id: 'unrelated', date: '2026-09-16', summary: '其他關聯', palaceMemoryId: 'other-node' },
    ];
    await DB.saveCharacter({ id: node.charId, name: '測試', memories } as CharacterProfile);
...
describe('palace deletion archive choice', () => {
    ...
    it.each([
        ...
        expect(dialog.textContent).toContain('神經鏈接的日度記憶中，有該記憶的備份');
