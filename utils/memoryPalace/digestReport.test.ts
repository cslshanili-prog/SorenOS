...
function report(i: number, charId = 'char_dr_test'): DigestReport {
    return {
        ...
        examined: [{ label: '閣樓困惑', items: [`困惑 ${i}`] }],
        outcomes: [],
        plateSubmissions: [{ label: '提交給門牌「TA的事」', items: [`事實 ${i}`] }],
        plateUpdated: ['user_room'],
    };
}

describe('DigestReportDB — 消化日誌存取與修剪', () => {
    it('按時間倒序返回，最新在前', async () => {
    ...
    it(`每角色只保留最近 ${DIGEST_REPORT_KEEP} 條，舊的被修剪`, async () => {
