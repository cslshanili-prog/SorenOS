...
describe('EventBox Light Index M1.3', () => {
    it('finds a box name and returns its summary representative', () => {
        const summary = node('summary-exam', '這段記憶總結了考試準備和最終成績。', {
            eventBoxId: 'box-exam',
            isBoxSummary: true,
            importance: 8,
        });
        const index = buildEventBoxLightIndex([summary], [box()]);
        const lookup = lookupEventBoxLightCandidates(index, [eventQuery('霧港觀測員資格考試')]);
    ...
    it('matches a Router event update against an overlapping box name', () => {
        const summary = node('summary-exam', '考試一路的進展。', {
            eventBoxId: 'box-exam',
            isBoxSummary: true,
        });
        const index = buildEventBoxLightIndex([summary], [box()]);
        const lookup = lookupEventBoxLightCandidates(index, [eventQuery('霧港觀測員成績')]);
    ...
    it('indexes entities stored on the summary node', () => {
        const summary = node('summary-exam', '項目進展已經壓縮進摘要。', {
            eventBoxId: 'box-exam',
            isBoxSummary: true,
            entities: [{ name: '霧港工坊', type: 'organization' }],
        });
        const index = buildEventBoxLightIndex([summary], [box({ name: '第一次合作' })]);
        const lookup = lookupEventBoxLightCandidates(index, [eventQuery('霧港工坊')]);
    ...
    it('uses the strongest live node when an uncompressed box has no summary', () => {
        const older = node('live-old', '霧嵐提過搬家的安排。', {
            eventBoxId: 'box-move',
            entities: [{ name: '霧嵐', type: 'person' }],
            importance: 4,
            createdAt: 100,
        });
        const stronger = node('live-strong', '後來重新確認了搬家日期。', {
            eventBoxId: 'box-move',
            tags: ['搬家計劃'],
            importance: 9,
            createdAt: 200,
        ...
        const lookup = lookupEventBoxLightCandidates(index, [eventQuery('霧嵐')]);
    ...
    it('ignores a query that contains only generic continuation words', () => {
        const summary = node('summary-exam', '考試後來有了結果。', {
            eventBoxId: 'box-exam',
            isBoxSummary: true,
        });
        const index = buildEventBoxLightIndex([summary], [box()]);

        expect(lookupEventBoxLightCandidates(index, [eventQuery('那個事情後來呢')]).candidates)
            .toEqual([]);
    });
    ...
    it('adds metadata candidates without deleting ordinary recall results or making them hard guarantees', () => {
        const ordinary: ScoredMemory[] = [
            {
                node: node('semantic-a', '舊 hybrid recall A'),
                finalScore: 0.82,
                similarity: 0.8,
                bm25Score: 0.2,
                roomScore: 0.82,
            ...
            {
                node: node('semantic-b', '舊 hybrid recall B'),
                finalScore: 0.74,
                similarity: 0.7,
                bm25Score: 0.1,
                roomScore: 0.74,
        ...
        const summary = node('summary-exam', '考試事件盒代表節點', {
            eventBoxId: 'box-exam',
            isBoxSummary: true,
        });
